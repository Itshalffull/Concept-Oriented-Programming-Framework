...
enrichmentRendererCommand
  .command('register')
  .description('Register a handler for an enrichment key in a given format . 
...
enrichmentRendererCommand
  .command('render')
  .description('Render enrichment content in the given format . Parses the 
...
enrichmentRendererCommand
  ...
  .option('--json', 'Output as JSON')
  .action(async (opts) => {
    const result = await globalThis.kernel.handleRequest({ method: 'listPatterns', ...opts });
    console.log(opts.json ? JSON.stringify(result) : result);
  });
