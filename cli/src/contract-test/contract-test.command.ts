...
export const contractTestCommand = new Command('contract-test')
  .description('Verify cross target interoperability for concepts that generate code in multiple languages Maintain contract definitions derived from concept specs Track which producer consumer pairs have been verified Gate deployment on contract verification passing');

contractTestCommand
  .command('generate')
  .description('Generate a contract definition from the concept spec The contract captures the wire level interface : action signatures , input output schemas , and expected variants Language independent the same contract applies to every target implementation')
...
contractTestCommand
  .command('verify')
  .description('Run contract verification : the consumer sends requests to the producer and checks that responses match the contract definition Tests serialization round trips , variant handling , error propagation , and edge cases')
...
contractTestCommand
  .command('matrix')
  .description('Return cross target verification matrix Shows which language pairs have been verified for which concepts')
...
contractTestCommand
  .command('can-deploy')
  .description('Check if deploying this concept in this language is safe all contract pairs involving this language have been verified for the current spec version')
  .requiredOption('--concept <concept>', 'Concept')
