...
/**
 * Build an MCP configuration string for a given format.
...
function buildMcpConfigString(
  ...
  if (format === 'claude-mcp-config') {
    ...
    // MCP servers start before/in parallel with SessionStart hooks, so
    // dependency installation must be part of the server command itself.
    // All npm output is silenced to keep stdout clean for JSON-RPC.
    const npmInstall = '{ npm install --prefer-offline --no-audit --no-fund || npm install; } >/dev/null 2>&1';
    const serverCmd = `npx tsx ${outputDir}/tools.ts ${manifestPath}`;
    return JSON.stringify({
      mcpServers: {
        [name]: {
          command: 'bash',
          args: ['-c', `${npmInstall}; exec ${serverCmd}`],
        },
      },
    }, null, 2);
  }
  ...
  if (format === 'codex-mcp-config') {
    ...
    // Same bootstrap pattern: install deps before starting the server.
    const npmInstall = '{ npm install --prefer-offline --no-audit --no-fund || npm install; } >/dev/null 2>&1';
    const serverCmd = `npx tsx ${outputDir}/tools.ts ${manifestPath}`;
    const lines = [
      `# Auto-generated by Clef Bind — MCP server configuration`,
      `# ${description}`,
      ``,
      `[mcp_servers.${name.replace(/-/g, '_')}]`,
      `command = "bash"`,
      `args = ${JSON.stringify(['-c', `${npmInstall}; exec ${serverCmd}`])}`,
    ];
    return lines.join('\n') + '\n';
  }

  if (format === 'gemini-mcp-config') {
    // Gemini CLI: settings.json with mcpServers
    // Same bootstrap pattern: install deps before starting the server.
    const npmInstall = '{ npm install --prefer-offline --no-audit --no-fund || npm install; } >/dev/null 2>&1';
    const serverCmd = `npx tsx ${outputDir}/tools.ts ${manifestPath}`;
    return JSON.stringify({
  ...
  if (format === 'claude-settings-config') {
    ...
    // Dependencies are now installed by the MCP server command itself
    // (see claude-mcp-config format), so this hook serves as a
    // belt-and-suspenders ensure for non-MCP tools that need node_modules.
    return JSON.stringify({
      hooks: {
        SessionStart: [
          {
  ...
  // Default: generic JSON with bootstrap
  const npmInstall = '{ npm install --prefer-offline --no-audit --no-fund || npm install; } >/dev/null 2>&1';
  const serverCmd = `npx tsx ${outputDir}/tools.ts ${manifestPath}`;
  return JSON.stringify({
    name,
  ...
  for (const [targetName, targetConfig] of Object.entries(targets)) {
    ...
    if (configOutputs && configOutputs.length > 0) {
      ...
      for (const configOut of configOutputs) {
        ...
        const mcpConfigContent = buildMcpConfigString(
          configOut.format,
          targetConfig,
