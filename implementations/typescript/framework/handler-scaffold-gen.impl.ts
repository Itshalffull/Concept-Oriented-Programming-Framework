// ============================================================
// HandlerScaffoldGen — Concept handler (.impl.ts) scaffold generator
//
...
function buildConformanceTest(input: Record<string, unknown>): string {
  ...
  const lines: string[] = [
    ...
    `import { ${toCamel(conceptName)}Handler } from '../implementations/typescript/framework/${kebab}.impl.js';`,
  ...
  async generate(input: Record<string, unknown>, _storage: ConceptStorage) {
    ...
    try {
      ...
      const files: { path: string; content: string }[] = [
        {
          path: `implementations/typescript/framework/${kebab}.impl.ts`,
          content: handlerCode,
        },
      ];
