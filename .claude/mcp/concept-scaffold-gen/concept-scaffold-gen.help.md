...
- **Invariant Completeness:** Use all seven invariant constructs comprehensively: example (named tests), forall (quantified properties), always (state predicates), never (safety), eventually (liveness), action requires/ensures (contracts), scenario (multi-block fixtures + given/when/then + settlement). Cover core purpose, error paths, constraints, state transitions, and boundary conditions. Aim for 2-5 invariants per concept. The same grammar is shared across .concept, .widget, .view, .sync, and .derived specs via the universal InvariantParser.
