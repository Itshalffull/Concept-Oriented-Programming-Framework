...
```bash
clef kit check-overrides ./kits/my-kit
```
...
|--------|---------|---------|
| init | `clef kit init <name>` | Scaffold a new kit |
| validate | `clef kit validate <path>` | Validate kit manifest |
| test | `clef kit test <path>` | Run kit tests |
| list | `clef kit list` | List active kits |
| checkOverrides | `clef kit check-overrides <path>` | Verify sync overrides |
