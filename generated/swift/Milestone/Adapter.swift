// generated: Milestone/Adapter.swift

import Foundation

class MilestoneAdapter: ConceptTransport {
    private let handler: any MilestoneHandler
    private let storage: ConceptStorage

    init(handler: any MilestoneHandler, storage: ConceptStorage) {
    ...
    func invoke(invocation: ActionInvocation) async throws -> ActionCompletion {
        ...
        case "define":
            let input = try decoder.decode(MilestoneDefineInput.self, from: invocation.inputData)
        ...
        case "evaluate":
            let input = try decoder.decode(MilestoneEvaluateInput.self, from: invocation.inputData)
        ...
        case "revoke":
            let input = try decoder.decode(MilestoneRevokeInput.self, from: invocation.inputData)
            let output = try await handler.revoke(input: input, storage: storage)
            let outputData = try encoder.encode(output)
            return ActionCompletion(
                id: invocation.id,
                concept: invocation.concept,
