// generated: Comment/Adapter.swift

import Foundation

class CommentAdapter: ConceptTransport {
    private let handler: any CommentHandler
    private let storage: ConceptStorage

    init(handler: any CommentHandler, storage: ConceptStorage) {
    ...
    func invoke(invocation: ActionInvocation) async throws -> ActionCompletion {
        ...
        case "create":
            let input = try decoder.decode(CommentCreateInput.self, from: invocation.inputData)
        ...
        case "delete":
            let input = try decoder.decode(CommentDeleteInput.self, from: invocation.inputData)
            let output = try await handler.delete(input: input, storage: storage)
            let outputData = try encoder.encode(output)
            return ActionCompletion(
                id: invocation.id,
                concept: invocation.concept,
        ...
        case "list":
            let input = try decoder.decode(CommentListInput.self, from: invocation.inputData)
