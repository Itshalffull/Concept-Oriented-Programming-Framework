// generated: Telemetry/Adapter.swift

import Foundation

class TelemetryAdapter: ConceptTransport {
    private let handler: any TelemetryHandler
    private let storage: ConceptStorage

    init(handler: any TelemetryHandler, storage: ConceptStorage) {
    ...
    func invoke(invocation: ActionInvocation) async throws -> ActionCompletion {
        ...
        case "export":
            let input = try decoder.decode(TelemetryExportInput.self, from: invocation.inputData)
        ...
        case "configure":
            let input = try decoder.decode(TelemetryConfigureInput.self, from: invocation.inputData)
            let output = try await handler.configure(input: input, storage: storage)
            let outputData = try encoder.encode(output)
            return ActionCompletion(
                id: invocation.id,
                concept: invocation.concept,
