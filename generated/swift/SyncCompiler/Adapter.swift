// generated: SyncCompiler/Adapter.swift

import Foundation

class SyncCompilerAdapter: ConceptTransport {
    private let handler: any SyncCompilerHandler
    private let storage: ConceptStorage

    init(handler: any SyncCompilerHandler, storage: ConceptStorage) {
    ...
    func invoke(invocation: ActionInvocation) async throws -> ActionCompletion {
        ...
        case "compile":
            let input = try decoder.decode(SyncCompilerCompileInput.self, from: invocation.inputData)
            let output = try await handler.compile(input: input, storage: storage)
