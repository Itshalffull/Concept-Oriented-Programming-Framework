// generated: FlowTrace/Adapter.swift

import Foundation

class FlowTraceAdapter: ConceptTransport {
    private let handler: any FlowTraceHandler
    private let storage: ConceptStorage

    init(handler: any FlowTraceHandler, storage: ConceptStorage) {
    ...
    func invoke(invocation: ActionInvocation) async throws -> ActionCompletion {
        ...
        case "build":
            let input = try decoder.decode(FlowTraceBuildInput.self, from: invocation.inputData)
            let output = try await handler.build(input: input, storage: storage)
        ...
        case "render":
            let input = try decoder.decode(FlowTraceRenderInput.self, from: invocation.inputData)
