...
describe('HierarchicalLayout functional handler', () => {
  ...
  describe('apply', () => {
    it('builds a valid StorageProgram', () => {
      const program = hierarchicalLayoutHandler.apply({ canvas: "c1", items: ["root","child1","child2"] });
      expect(program).toBeDefined();
      expect(program.instructions).toBeDefined();
      expect(Array.isArray(program.instructions)).toBe(true);
      expect(program.instructions.length).toBeGreaterThan(0);
    ...
    it('fixture "missing_canvas" -> error', async () => {
      if (typeof hierarchicalLayoutHandler.apply !== 'function') return;
      const storage = createInMemoryStorage();
      const result = await interpret(hierarchicalLayoutHandler.apply({ canvas: "", items: ["a"] }), storage);
  ...
  describe('register()', () => {
    it('declares concept name', async () => {
      ...
      expect(name).toBe('HierarchicalLayout');
    });
  });
  ...
  describe('invariant examples', () => {
    it("apply-then-apply", async () => {
      const storage = createInMemoryStorage();
      const applyResult0 = await interpret(hierarchicalLayoutHandler.apply({ canvas: "c1", items: ["root","child1","child2"] }), storage);
      expect(applyResult0.variant).toBe("ok");
      let positions = applyResult0.output["positions"];
      let _ = positions;
      const thenResult0 = await interpret(hierarchicalLayoutHandler.apply({ canvas: "c1", items: ["root"] }), storage);
