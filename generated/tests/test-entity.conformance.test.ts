...
describe('TestEntity functional handler', () => {
  ...
  describe('findFailing', () => {
    ...
    it('covers all declared variants', () => {
      ...
      const variants = extractCompletionVariants(program);
      expect(variants).toContain('ok');
      expect(variants).toContain('allPassing');
    });
  ...
  describe('untestedActions', () => {
    ...
    it('covers all declared variants', () => {
      ...
      const variants = extractCompletionVariants(program);
      expect(variants).toContain('ok');
      expect(variants).toContain('fullCoverage');
    });
    ...
    it('always: every tests entry has name', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(
            fc.oneof(
              fc.record({ action: fc.constant('register'), input: fc.record({ name: fc.string({ minLength: 1, maxLength: 20 }), sourceFile: fc.string({ minLength: 1, maxLength: 20 }), kind: fc.string({ minLength: 1, maxLength: 20 }), targetEntity: fc.string({ minLength: 1, maxLength: 20 }) }) }),
              fc.record({ action: fc.constant('get'), input: fc.record({ name: fc.string({ minLength: 1, maxLength: 20 }) }) }),
              fc.record({ action: fc.constant('findByEntity'), input: fc.record({ entity: fc.string({ minLength: 1, maxLength: 20 }) }) }),
              ...
              fc.record({ action: fc.constant('findByKind'), input: fc.record({ kind: fc.string({ minLength: 1, maxLength: 20 }) }) }),
              fc.record({ action: fc.constant('findFailing'), input: fc.record({  }) }),
              fc.record({ action: fc.constant('coverageReport'), input: fc.record({ entity: fc.string({ minLength: 1, maxLength: 20 }) }) }),
              ...
              fc.record({ action: fc.constant('recordResult'), input: fc.record({ test: fc.string(), result: fc.string({ minLength: 1, maxLength: 20 }), duration: fc.integer({ min: 0, max: 1000 }) }) }),
            ),
