...
describe('Patch functional handler', () => {
  ...
  describe('compose', () => {
    ...
    it('fixture "compose_sequential" -> ok', async () => {
      if (typeof patchHandler.compose !== 'function') return;
      const storage = createInMemoryStorage();
      // Create first patch: base -> mid
      const firstPatch = await interpret(patchHandler.create({ base: "sha256:aaa111", target: "sha256:bbb222", effect: "[{\"type\":\"equal\",\"line\":0,\"content\":\"hello\"}]" }), storage);
      // Create second patch: mid -> end (base must match first's target)
      const secondPatch = await interpret(patchHandler.create({ base: "sha256:bbb222", target: "sha256:ccc333", effect: "[{\"type\":\"equal\",\"line\":0,\"content\":\"hello\"}]" }), storage);
      const result = await interpret(patchHandler.compose({ first: firstPatch?.output?.["patchId"], second: secondPatch?.output?.["patchId"] }), storage);
      expect(result.variant).toBe('ok');
    });
  ...
  describe('invariant examples', () => {
    it("create then apply", async () => {
      const storage = createInMemoryStorage();
      const createResult0 = await interpret(patchHandler.create({ base: "sha256:base", target: "sha256:target", effect: "[{\"type\":\"equal\",\"line\":0,\"content\":\"hello\"}]" }), storage);
      expect(createResult0.variant).toBe("ok");
      let patchId = createResult0.output["patchId"];
      let p = patchId;
      const thenResult0 = await interpret(patchHandler.apply({ patchId: p, content: "hello" }), storage);
      expect(thenResult0.variant).toBe("ok");
    });

    it("invert then apply", async () => {
      const storage = createInMemoryStorage();
      const createResult0 = await interpret(patchHandler.create({ base: "sha256:base", target: "sha256:target", effect: "[{\"type\":\"insert\",\"line\":0,\"content\":\"hello\"}]" }), storage);
      expect(createResult0.variant).toBe("ok");
      let patchId = createResult0.output["patchId"] as string;
      const invertResult0 = await interpret(patchHandler.invert({ patchId: patchId }), storage);
      expect(invertResult0.variant).toBe("ok");
      let inversePatchId = invertResult0.output["inversePatchId"];
      let inv = inversePatchId;
      const thenResult0 = await interpret(patchHandler.apply({ patchId: patchId, content: "hello" }), storage);
      expect(thenResult0.variant).toBe("ok");
      const thenResult1 = await interpret(patchHandler.apply({ patchId: inv, content: "hello" }), storage);
