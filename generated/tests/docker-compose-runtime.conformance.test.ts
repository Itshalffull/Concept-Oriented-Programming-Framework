    ...
    it('always: valid-composePath', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(
            fc.oneof(
              fc.record({ action: fc.constant('provision'), input: fc.record({ concept: fc.string({ minLength: 1, maxLength: 20 }), composePath: fc.string({ minLength: 1, maxLength: 20 }), ports: fc.string() }) }),
              ...
              fc.record({ action: fc.constant('rollback'), input: fc.record({ service: fc.string(), targetImage: fc.string({ minLength: 1, maxLength: 20 }) }) }),
