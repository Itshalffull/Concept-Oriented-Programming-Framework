...
describe('TypeScriptGen functional handler', () => {
  ...
  describe('invariant examples', () => {
    it("generate valid manifest then invalid fails", async () => {
      const storage = createInMemoryStorage();
      const generateResult0 = await interpret(typeScriptGenHandler.generate({ spec: "s1", manifest: {"name":"Ping","uri":"urn:clef/Ping","typeParams":[],"relations":[],"actions":[{"name":"ping","params":[],"variants":[{"tag":"ok","fields":[],"prose":"Pong."}]}],"invariants":[],"graphqlSchema":"","jsonSchemas":{"invocations":{},"completions":{}},"capabilities":[],"purpose":"A test."} }), storage);
      expect(generateResult0.variant).toBe("ok");
      let files = generateResult0.output["files"];
      let f = files;
      const thenResult0 = await interpret(typeScriptGenHandler.generate({ spec: "s2", manifest: {"name":""} }), storage);
