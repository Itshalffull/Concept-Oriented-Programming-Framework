...
describe('GridLayout functional handler', () => {
  ...
  describe('apply', () => {
    ...
    it('fixture "valid_apply" -> ok', async () => {
      if (typeof gridLayoutHandler.apply !== 'function') return;
      const storage = createInMemoryStorage();
      const afterResult_valid = await interpret(gridLayoutHandler.register({  }), storage);
      const _pool = Object.assign({}, (afterResult_valid?.output ?? {}));
      const _fixtureInput = { canvas: "c1", items: ["a","b","c","d"] } as Record<string, unknown>;
  ...
  describe('invariant examples', () => {
    it("apply-then-apply", async () => {
      const storage = createInMemoryStorage();
      const applyResult0 = await interpret(gridLayoutHandler.apply({ canvas: "c1", items: ["a","b","c","d"] }), storage);
      expect(applyResult0.variant).toBe("ok");
      let positions = applyResult0.output["positions"];
      let p = positions;
      const thenResult0 = await interpret(gridLayoutHandler.apply({ canvas: "c1", items: ["a"] }), storage);
