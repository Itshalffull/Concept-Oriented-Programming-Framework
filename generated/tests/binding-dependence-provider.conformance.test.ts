// BindingDependenceProvider Functional Handler Conformance Tests
//
...
import { bindingDependenceProviderHandler } from '../../handlers/ts/binding-dependence-provider.handler.js';
import {
  classifyPurity,
...
describe('BindingDependenceProvider functional handler', () => {
  ...
  describe('initialize', () => {
    it('builds a valid StorageProgram', () => {
      const program = bindingDependenceProviderHandler.initialize({  });
  ...
  describe('register()', () => {
    it('declares concept name', async () => {
      ...
      const storage = createInMemoryStorage();
      const program = bindingDependenceProviderHandler.register({});
      ...
      expect(name).toBe('BindingDependenceProvider');
    });
  });
        ...
        fc.asyncProperty(
          ...
          async (actionSequence) => {
            ...
            for (const step of actionSequence) {
              const actionFn = bindingDependenceProviderHandler[step.action];
              if (typeof actionFn === 'function') {
                const result = await safeInvoke(async () => {
                  const program = actionFn.call(bindingDependenceProviderHandler, step.input as Record<string, unknown>);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (input) => {
            const storage = createInMemoryStorage();
            const result = await safeInvoke(async () => {
              const program = bindingDependenceProviderHandler.initialize(input as Record<string, unknown>);
              return interpret(program, storage);
            });
