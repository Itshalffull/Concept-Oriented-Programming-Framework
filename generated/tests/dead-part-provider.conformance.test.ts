...
describe('DeadPartProvider functional handler', () => {
  ...
  describe('getResults', () => {
    it('builds a valid StorageProgram', () => {
      const program = deadPartProviderHandler.getResults({ analysis: {"type":"ref","fixture":"all_parts_used","field":"analysis"} });
      expect(program).toBeDefined();
      expect(program.instructions).toBeDefined();
      expect(Array.isArray(program.instructions)).toBe(true);
      expect(program.instructions.length).toBeGreaterThan(0);
    ...
    it('produces a result', async () => {
      ...
      const result = await interpret(deadPartProviderHandler.getResults({ analysis: {"type":"ref","fixture":"all_parts_used","field":"analysis"} }), storage);
      expect(result).toBeDefined();
      if (result.variant !== undefined) {
        expect(typeof result.variant).toBe('string');
      }
  ...
  describe('invariant examples', () => {
    it("analyze stores retrievable dead part results", async () => {
      const storage = createInMemoryStorage();
      const analyzeResult0 = await interpret(deadPartProviderHandler.analyze({ analysis: "test-d", program: "p1", parts: ["root","unused"], instructions: ["element:root:container","text:root:hello"] }), storage);
