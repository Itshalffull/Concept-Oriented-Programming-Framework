    ...
    it('always: adapters have a backend', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(
            fc.oneof(
              fc.record({ action: fc.constant('generate'), input: fc.record({ name: fc.string({ minLength: 1, maxLength: 20 }), backend: fc.string({ minLength: 1, maxLength: 20 }) }) }),
              fc.record({ action: fc.constant('preview'), input: fc.record({ name: fc.string({ minLength: 1, maxLength: 20 }), backend: fc.string({ minLength: 1, maxLength: 20 }) }) }),
