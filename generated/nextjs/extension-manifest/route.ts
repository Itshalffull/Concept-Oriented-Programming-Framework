...
import type { ExtensionManifestHandler, ExtensionManifestError } from './handler.js';
import type { ExtensionManifestStorage } from './types.js';

const errorResponse = (error: ExtensionManifestError, status = 422): NextResponse =>
...
export const createExtensionManifestRoutes = (
  handler: ExtensionManifestHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'listDependencies': {
        const result = await handler.listDependencies(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
