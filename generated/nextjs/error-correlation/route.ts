...
import type { ErrorCorrelationHandler, ErrorCorrelationError } from './handler.js';
import type { ErrorCorrelationStorage } from './types.js';

const errorResponse = (error: ErrorCorrelationError, status = 422): NextResponse =>
...
export const createErrorCorrelationRoutes = (
  handler: ErrorCorrelationHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'findByEntity': {
        const result = await handler.findByEntity(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'errorHotspots': {
        const result = await handler.errorHotspots(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'rootCause': {
        const result = await handler.rootCause(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'findByEntity';
