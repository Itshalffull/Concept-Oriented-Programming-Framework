// VersionContext — route.ts
// Generated by NextjsGen. Do not edit manually.
// Uses fp-ts for purely functional, composable concept implementations.

// Next.js App Router Route Handler — maps HTTP to concept actions
// Place at: app/api/version-context/route.ts

import { NextRequest, NextResponse } from 'next/server';
import * as E from 'fp-ts/Either';
import { pipe } from 'fp-ts/function';

import type { VersionContextHandler, VersionContextError } from './handler.js';
import type { VersionContextStorage } from './types.js';

const errorResponse = (error: VersionContextError, status = 422): NextResponse =>
...
export const createVersionContextRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'push': {
        const result = await handler.push(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'pop': {
        const result = await handler.pop(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'resolveFor': {
        const result = await handler.resolveFor(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
