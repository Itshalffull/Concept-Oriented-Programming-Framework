...
// Place at: app/api/audit-trail/route.ts
...
import type { AuditTrailHandler, AuditTrailError } from './handler.js';
import type { AuditTrailStorage } from './types.js';

const errorResponse = (error: AuditTrailError, status = 422): NextResponse =>
...
export const createAuditTrailRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'verifyIntegrity': {
        const result = await handler.verifyIntegrity(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
