...
import type { SolverProviderHandler, SolverProviderError } from './handler.js';
import type { SolverProviderStorage } from './types.js';

const errorResponse = (error: SolverProviderError, status = 422): NextResponse =>
...
export const createSolverProviderRoutes = (
  handler: SolverProviderHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'dispatch_batch': {
        const result = await handler.dispatch_batch(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'health_check': {
        const result = await handler.health_check(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'unregister': {
        const result = await handler.unregister(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
