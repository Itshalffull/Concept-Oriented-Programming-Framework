...
import type { VariantEntityHandler, VariantEntityError } from './handler.js';
import type { VariantEntityStorage } from './types.js';

const errorResponse = (error: VariantEntityError, status = 422): NextResponse =>
...
export const createVariantEntityRoutes = (
  handler: VariantEntityHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'matchingSyncs': {
        const result = await handler.matchingSyncs(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'isDead': {
        const result = await handler.isDead(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
