...
import type { ZkSyncProviderHandler, ZkSyncProviderError } from './handler.js';
import type { ZkSyncProviderStorage } from './types.js';

const errorResponse = (error: ZkSyncProviderError, status = 422): NextResponse =>
...
export const createZkSyncProviderRoutes = (
  handler: ZkSyncProviderHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'getBatchProof': {
        const result = await handler.getBatchProof(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
