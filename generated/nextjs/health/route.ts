...
import type { HealthHandler, HealthError } from './handler.js';
import type { HealthStorage } from './types.js';

const errorResponse = (error: HealthError, status = 422): NextResponse =>
...
export const createHealthRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'checkConcept': {
        const result = await handler.checkConcept(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'checkSync': {
        const result = await handler.checkSync(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'checkKit': {
        const result = await handler.checkKit(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'checkInvariant': {
        const result = await handler.checkInvariant(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
