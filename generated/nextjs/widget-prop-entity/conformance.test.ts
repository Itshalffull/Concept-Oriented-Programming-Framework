// WidgetPropEntity — conformance.test.ts
...
describe('WidgetPropEntity conformance', () => {
  it('invariant 1: after register, get behaves correctly', async () => {
    ...
    const getResultAssert = await pipe(
      ...
      TE.map((output) => {
        ...
        expect((output as any).name).toBe('closeOnEscape');
        expect((output as any).typeExpr).toBe('Bool');
        expect((output as any).defaultValue).toBe('true');
        return output;
      }),
    )();
