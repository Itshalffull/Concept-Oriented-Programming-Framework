...
import type { WidgetPropEntityHandler, WidgetPropEntityError } from './handler.js';
import type { WidgetPropEntityStorage } from './types.js';

const errorResponse = (error: WidgetPropEntityError, status = 422): NextResponse =>
...
export const createWidgetPropEntityRoutes = (
  handler: WidgetPropEntityHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'traceToField': {
        const result = await handler.traceToField(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
