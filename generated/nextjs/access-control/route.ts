...
import type { AccessControlHandler, AccessControlError } from './handler.js';
import type { AccessControlStorage } from './types.js';

const errorResponse = (error: AccessControlError, status = 422): NextResponse =>
...
export const createAccessControlRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'orIf': {
        const result = await handler.orIf(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'andIf': {
        const result = await handler.andIf(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
