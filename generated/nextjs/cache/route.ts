...
// Place at: app/api/cache/route.ts
...
import type { CacheHandler, CacheError } from './handler.js';
import type { CacheStorage } from './types.js';

const errorResponse = (error: CacheError, status = 422): NextResponse =>
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'invalidateByTags': {
        const result = await handler.invalidateByTags(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
