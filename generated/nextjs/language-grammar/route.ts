...
import type { LanguageGrammarHandler, LanguageGrammarError } from './handler.js';
import type { LanguageGrammarStorage } from './types.js';

const errorResponse = (error: LanguageGrammarError, status = 422): NextResponse =>
...
export const createLanguageGrammarRoutes = (
  handler: LanguageGrammarHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'resolveByMime': {
        const result = await handler.resolveByMime(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
