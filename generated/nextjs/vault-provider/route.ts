...
import type { VaultProviderHandler, VaultProviderError } from './handler.js';
import type { VaultProviderStorage } from './types.js';

const errorResponse = (error: VaultProviderError, status = 422): NextResponse =>
...
export const createVaultProviderRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'renewLease': {
        const result = await handler.renewLease(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
