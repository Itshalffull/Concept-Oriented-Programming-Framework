...
import type { ConflictResolutionHandler, ConflictResolutionError } from './handler.js';
import type { ConflictResolutionStorage } from './types.js';

const errorResponse = (error: ConflictResolutionError, status = 422): NextResponse =>
...
export const createConflictResolutionRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'registerPolicy': {
        const result = await handler.registerPolicy(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'detect': {
        const result = await handler.detect(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'manualResolve': {
        const result = await handler.manualResolve(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
