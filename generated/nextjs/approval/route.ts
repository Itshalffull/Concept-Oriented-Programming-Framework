...
// Place at: app/api/approval/route.ts

import { NextRequest, NextResponse } from 'next/server';
import * as E from 'fp-ts/Either';
import { pipe } from 'fp-ts/function';

import type { ApprovalHandler, ApprovalError } from './handler.js';
import type { ApprovalStorage } from './types.js';

const errorResponse = (error: ApprovalError, status = 422): NextResponse =>
...
export const createApprovalRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'deny': {
        const result = await handler.deny(input as any, storage)();
        return pipe(result, E.fold((e) => errorResponse(e), (o) => successResponse(o)));
      }
      case 'request_changes': {
        const result = await handler.request_changes(input as any, storage)();
        return pipe(result, E.fold((e) => errorResponse(e), (o) => successResponse(o)));
      }
      case 'timeout': {
        const result = await handler.timeout(input as any, storage)();
        return pipe(result, E.fold((e) => errorResponse(e), (o) => successResponse(o)));
      }
      case 'get_status': {
        const result = await handler.get_status(input as any, storage)();
        return pipe(result, E.fold((e) => errorResponse(e), (o) => successResponse(o)));
      }
      default:
