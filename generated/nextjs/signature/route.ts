...
// Place at: app/api/signature/route.ts
...
import type { SignatureHandler, SignatureError } from './handler.js';
import type { SignatureStorage } from './types.js';

const errorResponse = (error: SignatureError, status = 422): NextResponse =>
...
export const createSignatureRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'sign': {
        const result = await handler.sign(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'timestamp': {
        const result = await handler.timestamp(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'addTrustedSigner': {
        const result = await handler.addTrustedSigner(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
