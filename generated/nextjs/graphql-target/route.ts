...
import type { GraphqlTargetHandler, GraphqlTargetError } from './handler.js';
import type { GraphqlTargetStorage } from './types.js';

const errorResponse = (error: GraphqlTargetError, status = 422): NextResponse =>
...
export const createGraphqlTargetRoutes = (
  handler: GraphqlTargetHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'listOperations': {
        const result = await handler.listOperations(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'listOperations';
