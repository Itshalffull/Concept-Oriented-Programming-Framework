...
// Place at: app/api/mcp-target/route.ts
...
import type { McpTargetHandler, McpTargetError } from './handler.js';
import type { McpTargetStorage } from './types.js';

const errorResponse = (error: McpTargetError, status = 422): NextResponse =>
...
export const createMcpTargetRoutes = (
  handler: McpTargetHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'listTools': {
        const result = await handler.listTools(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'listTools';
