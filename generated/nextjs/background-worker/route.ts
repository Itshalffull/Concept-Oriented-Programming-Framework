...
import type { BackgroundWorkerHandler, BackgroundWorkerError } from './handler.js';
import type { BackgroundWorkerStorage } from './types.js';

const errorResponse = (error: BackgroundWorkerError, status = 422): NextResponse =>
...
export const createBackgroundWorkerRoutes = (
  handler: BackgroundWorkerHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'setAlarm': {
        const result = await handler.setAlarm(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'clearAlarm': {
        const result = await handler.clearAlarm(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'onAlarm': {
        const result = await handler.onAlarm(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
