...
import type { RegoEvaluatorHandler, RegoEvaluatorError } from './handler.js';
import type { RegoEvaluatorStorage } from './types.js';

const errorResponse = (error: RegoEvaluatorError, status = 422): NextResponse =>
...
export const createRegoEvaluatorRoutes = (
  handler: RegoEvaluatorHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'loadBundle': {
        const result = await handler.loadBundle(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'updateData': {
        const result = await handler.updateData(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
