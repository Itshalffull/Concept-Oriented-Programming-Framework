...
// Place at: app/api/constitution/route.ts
...
import type { ConstitutionHandler, ConstitutionError } from './handler.js';
import type { ConstitutionStorage } from './types.js';

const errorResponse = (error: ConstitutionError, status = 422): NextResponse =>
...
export const createConstitutionRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'critiqueAndRevise': {
        const result = await handler.critiqueAndRevise(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'addPrinciple': {
        const result = await handler.addPrinciple(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'removePrinciple': {
        const result = await handler.removePrinciple(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
