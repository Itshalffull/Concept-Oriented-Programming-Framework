...
import type { DesignTokenHandler, DesignTokenError } from './handler.js';
import type { DesignTokenStorage } from './types.js';

const errorResponse = (error: DesignTokenError, status = 422): NextResponse =>
...
export const createDesignTokenRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'alias': {
        const result = await handler.alias(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
