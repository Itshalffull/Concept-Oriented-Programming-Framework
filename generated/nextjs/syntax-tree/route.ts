...
// Place at: app/api/syntax-tree/route.ts
...
import type { SyntaxTreeHandler, SyntaxTreeError } from './handler.js';
import type { SyntaxTreeStorage } from './types.js';

const errorResponse = (error: SyntaxTreeError, status = 422): NextResponse =>
...
export const createSyntaxTreeRoutes = (
  handler: SyntaxTreeHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'reparse': {
        const result = await handler.reparse(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'nodeAt': {
        const result = await handler.nodeAt(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
