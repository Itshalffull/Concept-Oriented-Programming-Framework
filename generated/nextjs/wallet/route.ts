...
// Place at: app/api/wallet/route.ts
...
import type { WalletHandler, WalletError } from './handler.js';
import type { WalletStorage } from './types.js';

const errorResponse = (error: WalletError, status = 422): NextResponse =>
...
export const createWalletRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'verifyTypedData': {
        const result = await handler.verifyTypedData(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'getNonce': {
        const result = await handler.getNonce(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'incrementNonce': {
        const result = await handler.incrementNonce(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'getNonce';
