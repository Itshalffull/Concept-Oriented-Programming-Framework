...
// Place at: app/api/scope-graph/route.ts
...
import type { ScopeGraphHandler, ScopeGraphError } from './handler.js';
import type { ScopeGraphStorage } from './types.js';

const errorResponse = (error: ScopeGraphError, status = 422): NextResponse =>
...
export const createScopeGraphRoutes = (
  handler: ScopeGraphHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'resolveReference': {
        const result = await handler.resolveReference(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'visibleSymbols': {
        const result = await handler.visibleSymbols(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'resolveCrossFile': {
        const result = await handler.resolveCrossFile(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
