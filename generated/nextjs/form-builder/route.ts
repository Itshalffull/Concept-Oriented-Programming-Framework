...
import type { FormBuilderHandler, FormBuilderError } from './handler.js';
import type { FormBuilderStorage } from './types.js';

const errorResponse = (error: FormBuilderError, status = 422): NextResponse =>
...
export const createFormBuilderRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'buildForm': {
        const result = await handler.buildForm(input as any, storage)();
        return pipe(
          result,
          E.fold(
