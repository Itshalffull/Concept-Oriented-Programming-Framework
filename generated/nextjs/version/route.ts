...
// Place at: app/api/version/route.ts
...
import type { VersionHandler, VersionError } from './handler.js';
import type { VersionStorage } from './types.js';

const errorResponse = (error: VersionError, status = 422): NextResponse =>
...
export const createVersionRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'listVersions': {
        const result = await handler.listVersions(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'listVersions';
