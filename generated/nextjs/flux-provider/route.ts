...
import type { FluxProviderHandler, FluxProviderError } from './handler.js';
import type { FluxProviderStorage } from './types.js';

const errorResponse = (error: FluxProviderError, status = 422): NextResponse =>
...
export const createFluxProviderRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'helmRelease': {
        const result = await handler.helmRelease(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
