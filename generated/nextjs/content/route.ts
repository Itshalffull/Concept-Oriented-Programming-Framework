...
// Place at: app/api/content/route.ts
...
import type { ContentHandler, ContentError } from './handler.js';
import type { ContentStorage } from './types.js';

const errorResponse = (error: ContentError, status = 422): NextResponse =>
...
export const createContentRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'unpin': {
        const result = await handler.unpin(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
