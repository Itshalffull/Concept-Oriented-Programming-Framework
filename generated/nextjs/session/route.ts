...
// Place at: app/api/session/route.ts
...
import type { SessionHandler, SessionError } from './handler.js';
import type { SessionStorage } from './types.js';

const errorResponse = (error: SessionError, status = 422): NextResponse =>
...
export const createSessionRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'destroyAll': {
        const result = await handler.destroyAll(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
