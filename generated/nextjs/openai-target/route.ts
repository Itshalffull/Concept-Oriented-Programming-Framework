...
import type { OpenaiTargetHandler, OpenaiTargetError } from './handler.js';
import type { OpenaiTargetStorage } from './types.js';

const errorResponse = (error: OpenaiTargetError, status = 422): NextResponse =>
...
export const createOpenaiTargetRoutes = (
  handler: OpenaiTargetHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'listFunctions': {
        const result = await handler.listFunctions(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'listFunctions';
