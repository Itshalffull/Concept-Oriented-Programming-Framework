...
import type { FileArtifactHandler, FileArtifactError } from './handler.js';
import type { FileArtifactStorage } from './types.js';

const errorResponse = (error: FileArtifactError, status = 422): NextResponse =>
...
export const createFileArtifactRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'setProvenance': {
        const result = await handler.setProvenance(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'findGeneratedFrom': {
        const result = await handler.findGeneratedFrom(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
