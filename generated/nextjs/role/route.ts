...
// Place at: app/api/role/route.ts
...
import type { RoleHandler, RoleError } from './handler.js';
import type { RoleStorage } from './types.js';

const errorResponse = (error: RoleError, status = 422): NextResponse =>
...
export const createRoleRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'assign': {
        const result = await handler.assign(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
