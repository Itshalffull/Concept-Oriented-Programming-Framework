...
// Place at: app/api/elevation/route.ts
...
import type { ElevationHandler, ElevationError } from './handler.js';
import type { ElevationStorage } from './types.js';

const errorResponse = (error: ElevationError, status = 422): NextResponse =>
...
export const createElevationRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'generateScale': {
        const result = await handler.generateScale(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
