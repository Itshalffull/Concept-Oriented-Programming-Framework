...
// Place at: app/api/validator/route.ts
...
import type { ValidatorHandler, ValidatorError } from './handler.js';
import type { ValidatorStorage } from './types.js';

const errorResponse = (error: ValidatorError, status = 422): NextResponse =>
...
export const createValidatorRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'registerConstraint': {
        const result = await handler.registerConstraint(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'validateField': {
        const result = await handler.validateField(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'addCustomValidator': {
        const result = await handler.addCustomValidator(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
