...
import type { WidgetStateEntityHandler, WidgetStateEntityError } from './handler.js';
import type { WidgetStateEntityStorage } from './types.js';

const errorResponse = (error: WidgetStateEntityError, status = 422): NextResponse =>
...
export const createWidgetStateEntityRoutes = (
  handler: WidgetStateEntityHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'findByWidget': {
        const result = await handler.findByWidget(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'reachableFrom': {
        const result = await handler.reachableFrom(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'unreachableStates': {
        const result = await handler.unreachableStates(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'traceEvent': {
        const result = await handler.traceEvent(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'findByWidget';
