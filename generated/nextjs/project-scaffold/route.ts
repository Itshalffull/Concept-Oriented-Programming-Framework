...
import type { ProjectScaffoldHandler, ProjectScaffoldError } from './handler.js';
import type { ProjectScaffoldStorage } from './types.js';

const errorResponse = (error: ProjectScaffoldError, status = 422): NextResponse =>
...
export const createProjectScaffoldRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'scaffold': {
        const result = await handler.scaffold(input as any, storage)();
        return pipe(
          result,
          E.fold(
