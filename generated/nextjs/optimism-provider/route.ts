...
import type { OptimismProviderHandler, OptimismProviderError } from './handler.js';
import type { OptimismProviderStorage } from './types.js';

const errorResponse = (error: OptimismProviderError, status = 422): NextResponse =>
...
export const createOptimismProviderRoutes = (
  handler: OptimismProviderHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'poll': {
        const result = await handler.poll(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'checkFinality': {
        const result = await handler.checkFinality(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'relayMessage': {
        const result = await handler.relayMessage(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
