...
// Place at: app/api/intent/route.ts
...
import type { IntentHandler, IntentError } from './handler.js';
import type { IntentStorage } from './types.js';

const errorResponse = (error: IntentError, status = 422): NextResponse =>
...
export const createIntentRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'suggestFromDescription': {
        const result = await handler.suggestFromDescription(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
