...
// Place at: app/api/evidence/route.ts
...
import type { EvidenceHandler, EvidenceError } from './handler.js';
import type { EvidenceStorage } from './types.js';

const errorResponse = (error: EvidenceError, status = 422): NextResponse =>
...
export const createEvidenceRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'minimize': {
        const result = await handler.minimize(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
