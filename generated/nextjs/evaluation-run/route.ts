...
import type { EvaluationRunHandler, EvaluationRunError } from './handler.js';
import type { EvaluationRunStorage } from './types.js';

const errorResponse = (error: EvaluationRunError, status = 422): NextResponse =>
...
export const createEvaluationRunRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'runEval': {
        const result = await handler.runEval(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'logMetric': {
        const result = await handler.logMetric(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'pass': {
        const result = await handler.pass(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'getResult': {
        const result = await handler.getResult(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'getResult';
