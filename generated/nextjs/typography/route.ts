...
// Place at: app/api/typography/route.ts
...
import type { TypographyHandler, TypographyError } from './handler.js';
import type { TypographyStorage } from './types.js';

const errorResponse = (error: TypographyError, status = 422): NextResponse =>
...
export const createTypographyRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'defineScale': {
        const result = await handler.defineScale(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'defineFontStack': {
        const result = await handler.defineFontStack(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'defineStyle': {
        const result = await handler.defineStyle(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
