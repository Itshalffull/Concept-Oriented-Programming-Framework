...
// Place at: app/api/alias/route.ts
...
import type { AliasHandler, AliasError } from './handler.js';
import type { AliasStorage } from './types.js';

const errorResponse = (error: AliasError, status = 422): NextResponse =>
...
export const createAliasRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'addAlias': {
        const result = await handler.addAlias(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'removeAlias': {
        const result = await handler.removeAlias(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
