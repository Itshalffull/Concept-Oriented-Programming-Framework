...
import type { ConceptEntityHandler, ConceptEntityError } from './handler.js';
import type { ConceptEntityStorage } from './types.js';

const errorResponse = (error: ConceptEntityError, status = 422): NextResponse =>
...
export const createConceptEntityRoutes = (
  handler: ConceptEntityHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'findByCapability': {
        const result = await handler.findByCapability(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'findByKit': {
        const result = await handler.findByKit(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'generatedArtifacts': {
        const result = await handler.generatedArtifacts(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'participatingSyncs': {
        const result = await handler.participatingSyncs(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'checkCompatibility': {
        const result = await handler.checkCompatibility(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
