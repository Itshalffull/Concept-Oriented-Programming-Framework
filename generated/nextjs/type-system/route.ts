...
// Place at: app/api/type-system/route.ts
...
import type { TypeSystemHandler, TypeSystemError } from './handler.js';
import type { TypeSystemStorage } from './types.js';

const errorResponse = (error: TypeSystemError, status = 422): NextResponse =>
...
export const createTypeSystemRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'registerType': {
        const result = await handler.registerType(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'navigate': {
        const result = await handler.navigate(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
