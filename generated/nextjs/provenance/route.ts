...
// Place at: app/api/provenance/route.ts
...
import type { ProvenanceHandler, ProvenanceError } from './handler.js';
import type { ProvenanceStorage } from './types.js';

const errorResponse = (error: ProvenanceError, status = 422): NextResponse =>
...
export const createProvenanceRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'reproduce': {
        const result = await handler.reproduce(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
