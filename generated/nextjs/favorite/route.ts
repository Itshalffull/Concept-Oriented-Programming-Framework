...
// Place at: app/api/favorite/route.ts
...
import type { FavoriteHandler, FavoriteError } from './handler.js';
import type { FavoriteStorage } from './types.js';

const errorResponse = (error: FavoriteError, status = 422): NextResponse =>
...
export const createFavoriteRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'favorite': {
        const result = await handler.favorite(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'unfavorite': {
        const result = await handler.unfavorite(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'isFavorited': {
        const result = await handler.isFavorited(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
