...
// Place at: app/api/registry/route.ts
...
import type { RegistryHandler, RegistryError } from './handler.js';
import type { RegistryStorage } from './types.js';

const errorResponse = (error: RegistryError, status = 422): NextResponse =>
...
export const createRegistryRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'heartbeat': {
        const result = await handler.heartbeat(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
