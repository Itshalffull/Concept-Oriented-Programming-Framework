...
// Place at: app/api/adapter/route.ts
...
import type { AdapterHandler, AdapterError } from './handler.js';
import type { AdapterStorage } from './types.js';

const errorResponse = (error: AdapterError, status = 422): NextResponse =>
...
export const createAdapterRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'train': {
        const result = await handler.train(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'swap': {
        const result = await handler.swap(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
