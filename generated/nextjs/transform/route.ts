...
// Place at: app/api/transform/route.ts
...
import type { TransformHandler, TransformError } from './handler.js';
import type { TransformStorage } from './types.js';

const errorResponse = (error: TransformError, status = 422): NextResponse =>
...
export const createTransformRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'chain': {
        const result = await handler.chain(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
