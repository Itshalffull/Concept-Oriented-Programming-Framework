...
// Place at: app/api/reputation/route.ts
...
import type { ReputationHandler, ReputationError } from './handler.js';
import type { ReputationStorage } from './types.js';

const errorResponse = (error: ReputationError, status = 422): NextResponse =>
...
export const createReputationRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'earn': {
        const result = await handler.earn(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'burn': {
        const result = await handler.burn(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'decay': {
        const result = await handler.decay(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'recalculate': {
        const result = await handler.recalculate(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
