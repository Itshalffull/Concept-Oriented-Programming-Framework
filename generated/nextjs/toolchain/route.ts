...
// Place at: app/api/toolchain/route.ts
...
import type { ToolchainHandler, ToolchainError } from './handler.js';
import type { ToolchainStorage } from './types.js';

const errorResponse = (error: ToolchainError, status = 422): NextResponse =>
...
export const createToolchainRoutes = (
  handler: ToolchainHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'capabilities': {
        const result = await handler.capabilities(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
