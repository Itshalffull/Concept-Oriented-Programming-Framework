...
import type { AgentHandoffHandler, AgentHandoffError } from './handler.js';
import type { AgentHandoffStorage } from './types.js';

const errorResponse = (error: AgentHandoffError, status = 422): NextResponse =>
...
export const createAgentHandoffRoutes = (
  handler: AgentHandoffHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'prepare': {
        const result = await handler.prepare(input as any, storage)();
        return pipe(
          result,
          E.fold(
