...
// Place at: app/api/collection/route.ts
...
import type { CollectionHandler, CollectionError } from './handler.js';
import type { CollectionStorage } from './types.js';

const errorResponse = (error: CollectionError, status = 422): NextResponse =>
...
export const createCollectionRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'getMembers': {
        const result = await handler.getMembers(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'setSchema': {
        const result = await handler.setSchema(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'getMembers';
