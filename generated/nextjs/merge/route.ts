...
// Place at: app/api/merge/route.ts
...
import type { MergeHandler, MergeError } from './handler.js';
import type { MergeStorage } from './types.js';

const errorResponse = (error: MergeError, status = 422): NextResponse =>
...
export const createMergeRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'registerStrategy': {
        const result = await handler.registerStrategy(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'finalize': {
        const result = await handler.finalize(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
