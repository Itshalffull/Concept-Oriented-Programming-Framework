...
// Place at: app/api/generator/route.ts
...
import type { GeneratorHandler, GeneratorError } from './handler.js';
import type { GeneratorStorage } from './types.js';

const errorResponse = (error: GeneratorError, status = 422): NextResponse =>
...
export const createGeneratorRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'regenerate': {
        const result = await handler.regenerate(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
