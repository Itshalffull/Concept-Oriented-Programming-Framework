...
// Place at: app/api/resource/route.ts
...
import type { ResourceHandler, ResourceError } from './handler.js';
import type { ResourceStorage } from './types.js';

const errorResponse = (error: ResourceError, status = 422): NextResponse =>
...
export const createResourceRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'upsert': {
        const result = await handler.upsert(input as any, storage)();
        return pipe(
          result,
          E.fold(
