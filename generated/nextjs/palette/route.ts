...
// Place at: app/api/palette/route.ts
...
import type { PaletteHandler, PaletteError } from './handler.js';
import type { PaletteStorage } from './types.js';

const errorResponse = (error: PaletteError, status = 422): NextResponse =>
...
export const createPaletteRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'checkContrast': {
        const result = await handler.checkContrast(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
