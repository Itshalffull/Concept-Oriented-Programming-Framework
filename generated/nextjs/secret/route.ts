...
// Place at: app/api/secret/route.ts
...
import type { SecretHandler, SecretError } from './handler.js';
import type { SecretStorage } from './types.js';

const errorResponse = (error: SecretError, status = 422): NextResponse =>
...
export const createSecretRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'exists': {
        const result = await handler.exists(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'invalidateCache': {
        const result = await handler.invalidateCache(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
