...
import type { ThemeEntityHandler, ThemeEntityError } from './handler.js';
import type { ThemeEntityStorage } from './types.js';

const errorResponse = (error: ThemeEntityError, status = 422): NextResponse =>
...
export const createThemeEntityRoutes = (
  handler: ThemeEntityHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'resolveToken': {
        const result = await handler.resolveToken(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'contrastAudit': {
        const result = await handler.contrastAudit(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'diffThemes': {
        const result = await handler.diffThemes(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'affectedWidgets': {
        const result = await handler.affectedWidgets(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'generatedOutputs': {
        const result = await handler.generatedOutputs(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
