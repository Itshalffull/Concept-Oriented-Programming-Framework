...
import type { PlatformAdapterHandler, PlatformAdapterError } from './handler.js';
import type { PlatformAdapterStorage } from './types.js';

const errorResponse = (error: PlatformAdapterError, status = 422): NextResponse =>
...
export const createPlatformAdapterRoutes = (
  handler: PlatformAdapterHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'mapNavigation': {
        const result = await handler.mapNavigation(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'mapZone': {
        const result = await handler.mapZone(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'handlePlatformEvent': {
        const result = await handler.handlePlatformEvent(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
