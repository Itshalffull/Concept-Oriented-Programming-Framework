...
// Place at: app/api/cli-target/route.ts
...
import type { CliTargetHandler, CliTargetError } from './handler.js';
import type { CliTargetStorage } from './types.js';

const errorResponse = (error: CliTargetError, status = 422): NextResponse =>
...
export const createCliTargetRoutes = (
  handler: CliTargetHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'listCommands': {
        const result = await handler.listCommands(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'listCommands';
