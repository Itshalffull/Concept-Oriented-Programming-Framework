...
import type { StructuralPatternHandler, StructuralPatternError } from './handler.js';
import type { StructuralPatternStorage } from './types.js';

const errorResponse = (error: StructuralPatternError, status = 422): NextResponse =>
...
export const createStructuralPatternRoutes = (
  handler: StructuralPatternHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'matchProject': {
        const result = await handler.matchProject(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
