...
// Place at: app/api/api-surface/route.ts
...
import type { ApiSurfaceHandler, ApiSurfaceError } from './handler.js';
import type { ApiSurfaceStorage } from './types.js';

const errorResponse = (error: ApiSurfaceError, status = 422): NextResponse =>
...
export const createApiSurfaceRoutes = (
  handler: ApiSurfaceHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'entrypoint': {
        const result = await handler.entrypoint(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
