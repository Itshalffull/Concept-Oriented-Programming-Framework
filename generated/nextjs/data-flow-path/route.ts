...
import type { DataFlowPathHandler, DataFlowPathError } from './handler.js';
import type { DataFlowPathStorage } from './types.js';

const errorResponse = (error: DataFlowPathError, status = 422): NextResponse =>
...
export const createDataFlowPathRoutes = (
  handler: DataFlowPathHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'traceFromConfig': {
        const result = await handler.traceFromConfig(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'traceToOutput': {
        const result = await handler.traceToOutput(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
