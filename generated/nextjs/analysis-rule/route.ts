...
import type { AnalysisRuleHandler, AnalysisRuleError } from './handler.js';
import type { AnalysisRuleStorage } from './types.js';

const errorResponse = (error: AnalysisRuleError, status = 422): NextResponse =>
...
export const createAnalysisRuleRoutes = (
  handler: AnalysisRuleHandler,
  storage: AnalysisRuleStorage,
) => {
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'evaluateAll': {
        const result = await handler.evaluateAll(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
