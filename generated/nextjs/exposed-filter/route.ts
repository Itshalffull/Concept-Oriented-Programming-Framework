...
import type { ExposedFilterHandler, ExposedFilterError } from './handler.js';
import type { ExposedFilterStorage } from './types.js';

const errorResponse = (error: ExposedFilterError, status = 422): NextResponse =>
...
export const createExposedFilterRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'expose': {
        const result = await handler.expose(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'collectInput': {
        const result = await handler.collectInput(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'applyToQuery': {
        const result = await handler.applyToQuery(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'resetToDefaults': {
        const result = await handler.resetToDefaults(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
