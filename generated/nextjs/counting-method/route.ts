...
import type { CountingMethodHandler, CountingMethodError } from './handler.js';
import type { CountingMethodStorage } from './types.js';

const errorResponse = (error: CountingMethodError, status = 422): NextResponse =>
...
export const createCountingMethodRoutes = (
  handler: CountingMethodHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'deregister': {
        const result = await handler.deregister(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
