...
// Place at: app/api/formula/route.ts
...
import type { FormulaHandler, FormulaError } from './handler.js';
import type { FormulaStorage } from './types.js';

const errorResponse = (error: FormulaError, status = 422): NextResponse =>
...
export const createFormulaRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'setExpression': {
        const result = await handler.setExpression(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'getDependencies';
