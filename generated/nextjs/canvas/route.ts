...
// Place at: app/api/canvas/route.ts
...
import type { CanvasHandler, CanvasError } from './handler.js';
import type { CanvasStorage } from './types.js';

const errorResponse = (error: CanvasError, status = 422): NextResponse =>
...
export const createCanvasRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'moveNode': {
        const result = await handler.moveNode(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'groupNodes': {
        const result = await handler.groupNodes(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
