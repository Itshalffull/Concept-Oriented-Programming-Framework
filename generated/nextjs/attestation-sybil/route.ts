...
import type { AttestationSybilHandler, AttestationSybilError } from './handler.js';
import type { AttestationSybilStorage } from './types.js';

const errorResponse = (error: AttestationSybilError, status = 422): NextResponse =>
...
export const createAttestationSybilRoutes = (
  handler: AttestationSybilHandler,
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'checkParticipant': {
        const result = await handler.checkParticipant(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
