...
// Place at: app/api/sync-engine/route.ts
...
import type { SyncEngineHandler, SyncEngineError } from './handler.js';
import type { SyncEngineStorage } from './types.js';

const errorResponse = (error: SyncEngineError, status = 422): NextResponse =>
...
export const createSyncEngineRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'registerSync': {
        const result = await handler.registerSync(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'onCompletion': {
        const result = await handler.onCompletion(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'evaluateWhere': {
        const result = await handler.evaluateWhere(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'queueSync': {
        const result = await handler.queueSync(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'onAvailabilityChange': {
        const result = await handler.onAvailabilityChange(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'drainConflicts': {
        const result = await handler.drainConflicts(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
