...
import type { ChangeStreamHandler, ChangeStreamError } from './handler.js';
import type { ChangeStreamStorage } from './types.js';

const errorResponse = (error: ChangeStreamError, status = 422): NextResponse =>
...
export const createChangeStreamRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'acknowledge': {
        const result = await handler.acknowledge(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'replay': {
        const result = await handler.replay(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
