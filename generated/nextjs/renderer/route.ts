...
// Place at: app/api/renderer/route.ts
...
import type { RendererHandler, RendererError } from './handler.js';
import type { RendererStorage } from './types.js';

const errorResponse = (error: RendererError, status = 422): NextResponse =>
...
export const createRendererRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'autoPlaceholder': {
        const result = await handler.autoPlaceholder(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'mergeCacheability': {
        const result = await handler.mergeCacheability(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
