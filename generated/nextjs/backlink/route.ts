...
// Place at: app/api/backlink/route.ts
...
import type { BacklinkHandler, BacklinkError } from './handler.js';
import type { BacklinkStorage } from './types.js';

const errorResponse = (error: BacklinkError, status = 422): NextResponse =>
...
export const createBacklinkRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'getBacklinks': {
        const result = await handler.getBacklinks(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'getUnlinkedMentions': {
        const result = await handler.getUnlinkedMentions(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'getBacklinks';
