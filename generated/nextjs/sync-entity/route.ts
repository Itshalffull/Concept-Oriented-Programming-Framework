...
// Place at: app/api/sync-entity/route.ts
...
import type { SyncEntityHandler, SyncEntityError } from './handler.js';
import type { SyncEntityStorage } from './types.js';

const errorResponse = (error: SyncEntityError, status = 422): NextResponse =>
...
export const createSyncEntityRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'findTriggerableBy': {
        const result = await handler.findTriggerableBy(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'chainFrom': {
        const result = await handler.chainFrom(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'findDeadEnds': {
        const result = await handler.findDeadEnds(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'findOrphanVariants': {
        const result = await handler.findOrphanVariants(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
