...
// Place at: app/api/contract/route.ts
...
import type { ContractHandler, ContractError } from './handler.js';
import type { ContractStorage } from './types.js';

const errorResponse = (error: ContractError, status = 422): NextResponse =>
...
export const createContractRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'discharge': {
        const result = await handler.discharge(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
