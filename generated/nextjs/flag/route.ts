...
// Place at: app/api/flag/route.ts
...
import type { FlagHandler, FlagError } from './handler.js';
import type { FlagStorage } from './types.js';

const errorResponse = (error: FlagError, status = 422): NextResponse =>
...
export const createFlagRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      case 'flag': {
        const result = await handler.flag(input as any, storage)();
        return pipe(
          result,
          E.fold(
      ...
      case 'unflag': {
        const result = await handler.unflag(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'isFlagged': {
        const result = await handler.isFlagged(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'getCount': {
        const result = await handler.getCount(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
  ...
  const GET = async (request: NextRequest): Promise<NextResponse> => {
    ...
    const action = searchParams.get('action') ?? 'getCount';
