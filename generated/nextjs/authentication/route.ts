...
export const createAuthenticationRoutes = (
  ...
  const dispatch = async (action: string, input: Record<string, unknown>): Promise<NextResponse> => {
    switch (action) {
      ...
      case 'login': {
        const result = await handler.login(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'logout': {
        const result = await handler.logout(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'authenticate': {
        const result = await handler.authenticate(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
      ...
      case 'resetPassword': {
        const result = await handler.resetPassword(input as any, storage)();
        return pipe(
          result,
          E.fold(
            (error) => errorResponse(error),
