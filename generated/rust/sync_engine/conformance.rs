...
mod tests {
    ...
    async fn sync_engine_invariant_1() {
        ...
        let step1 = handler.register_sync(
            RegisterSyncInput { sync: todo!(/* record: { "name": "TestSync".to_string(), "annotations": todo!(/* list: ["eager".to_string()] */), "when": todo!(/* list: [todo!(/* record: { "concept": "urn:copf/Test".to_string(), "action": "act".to_string(), "inputFields": todo!(/* list: [] */), "outputFields": todo!(/* list: [] */) } */)] */), "where": todo!(/* list: [] */), "then": todo!(/* list: [todo!(/* record: { "concept": "urn:copf/Other".to_string(), "action": "do".to_string(), "fields": todo!(/* list: [] */) } */)] */) } */) },
            &storage,
        ).await.unwrap();
        ...
        let step2 = handler.on_completion(
            OnCompletionInput { completion: todo!(/* record: { "id": "c1".to_string(), "concept": "urn:copf/Test".to_string(), "action": "act".to_string(), "input": todo!(/* record: {  } */), "variant": "ok".to_string(), "output": todo!(/* record: {  } */), "flow": "f1".to_string(), "timestamp": "2024-01-01T00:00:00Z".to_string() } */) },
            &storage,
        ).await.unwrap();
        match step2 {
