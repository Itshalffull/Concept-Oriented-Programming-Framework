// generated: java_sdk_target/conformance.rs

#[cfg(test)]
mod tests {
    use super::super::handler::JavaSdkTargetHandler;
    use super::super::types::*;
    use crate::storage::create_in_memory_storage;

    #[tokio::test]
    async fn java_sdk_target_invariant_1() {
        ...
        // --- AFTER clause ---
        // generate(projection: "test-projection", config: "{}") -> ok(artifact: s, files: f)
        let step1 = handler.generate(
            GenerateInput { projection: "test-projection".to_string(), config: "{}".to_string() },
            &storage,
        ).await.unwrap();
        match step1 {
            GenerateOutput::Ok { artifact, files, .. } => {
                assert_eq!(artifact, s.clone());
        ...
        // --- THEN clause ---
        // generate(projection: "test-projection-2", config: "{}") -> ok(artifact: s2, files: f2)
