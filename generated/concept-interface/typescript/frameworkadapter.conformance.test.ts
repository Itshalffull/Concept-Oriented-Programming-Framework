...
describe("FrameworkAdapter conformance", () => {
  ...
  it("invariant 1: after register with normalizer/mountFn, normalize behaves correctly", async () => {
    const storage = createInMemoryStorage();

    const r = "u-test-invariant-001";

    // --- AFTER clause ---
    // register(renderer: r, framework: "react", version: "19",
    ...
    // --- THEN clause ---
    // normalize(renderer: r, props: '{ "onClick": "handler_1" }')
    //   -> ok(normalized: _)
  ...
  it("render lifecycle", async () => {
    const storage = createInMemoryStorage();

    const r = "u-test-render-001";

    await frameworkadapterHandler.register(
      { renderer: r, framework: "solid", version: "1.8",
        normalizer: "solidNormalizer", mountFn: "solidMount" },
      storage,
    );

    const renderResult = await frameworkadapterHandler.render(
      { renderer: r, tree: '{"root":"app"}', target: "#app" },
      storage,
    );
    expect(renderResult.variant).toBe("ok");
    expect((renderResult as any).renderer).toBe(r);
  });
