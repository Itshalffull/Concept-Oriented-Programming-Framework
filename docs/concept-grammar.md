...
### What scenario invariants drive

Beyond the standard TestPlan → renderer pipeline (React + Playwright,
vitest), scenario invariants feed `IntegrationTestGen`, which emits
ProcessSpec steps against real external APIs:

- `when { … }` steps → external-call ProcessSpec steps.
- `then { … }` steps → `CheckVerification` assertions on the prior step.
- `settlement: sync` → default step execution.
- `settlement: "async-eventually" { timeoutMs: N }` → per-step `pollUntil`
  with the given timeout.
- `settlement: "async-with-anchor" { anchor: "..." }` → per-step
  `waitForCompletion` keyed on the anchor.

Fixture chains (`fixture … after …`) and scenario invariants coexist:
fixture-derived steps emit first, then scenario `whenSteps`/`thenSteps`
are appended to the same ProcessSpec. The same invariant also flows
through the standard TestPlan → renderer pipeline for in-process vitest
generation alongside the ProcessSpec emission.

## Handler-side test generation (automatic)

Every registered handler automatically participates in the test
generation pipeline — no additional spec-level authoring is required
beyond declaring the handler's effects through the normal
StorageProgram DSL (reads, writes, performs).

`TestGeneration/run` accepts an optional `handlerDescriptors` input
(a normalized `HandlerDescriptor` IR derived from StorageProgram effect
analysis — `read-write-set-provider` + `transport-effect-provider` — or
from ExternalHandler manifests) and dispatches each descriptor to four
renderer plugins in parallel:

| Renderer | What it generates |
|---|---|
| `MockHandlerRenderer` | A mock handler driven by the declared reads/writes. |
| `EffectContractRenderer` | A contract test verifying the declared effect set matches runtime behaviour. |
| `ReplayHandlerRenderer` | A replay harness keyed on recorded completions. |
| `FieldTransformFuzzRenderer` | A property-based fuzz test over declared field transforms. |

What to do at the spec level to make this work well: **declare your
handler's effects accurately through the normal StorageProgram DSL**.
The richer your declared effect set (including `traverse` declared
effects and `perform` transport effects), the stronger the
auto-generated contract.

ExternalHandler output (generated by `ExternalHandlerGen/generate`)
rounds-trips through this same pipeline: an ok-completion of
`ExternalHandlerGen/generate` fires `TestGeneration/run` for the
generated concept, producing the same four artifacts without manual
intervention.

Cross-link: see `repertoire/concepts/test-plan-renderers/` for the
plugin set and
`handlers/ts/repertoire/test-plan-renderers/handler-descriptor.ts` for
the normalized IR shape.
