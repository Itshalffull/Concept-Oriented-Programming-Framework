...
| [MAG-919](https://vibekanban.com) ✅ `d62c6301` | INV-15 Add external-handler-gen + integration-test-gen skills — extend `examples/devtools/devtools.interface.yaml`, regen via `scripts/regen-interface.ts` (skill slugs follow concept names per claude-skills target) | MAG-922 |
...
| [MAG-923](https://vibekanban.com) | INV-19 Propagate MAG-918 + MAG-920 implications into grammar docs — scenario kind drives IntegrationTestGen; every handler auto-participates in mock/effect-contract/replay/fuzz via HandlerDescriptor; ExternalHandlerGen output round-trips through TestGeneration | — |
