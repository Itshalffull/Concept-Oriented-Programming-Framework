...
Implement the Generation Suite per `concept-generate/clef-generation-suite.md`. Creates 5 new concepts (Resource, KindSystem, BuildCache, GenerationPlan, Emitter) in `suites/generation/`, with 14 shared syncs, TypeScript implementations, and conformance + integration tests. Follows the doc's 6-phase ordering: Emitter → BuildCache → Resource → KindSystem → GenerationPlan → Traceability.

## Existing State (what we're building on)

- **Emitter already exists** at `suites/interface/concepts/emitter.concept` with impl at `implementations/typescript/framework/emitter.impl.ts`. It has `write`, `format`, `clean`, `manifest` actions. The generation suite version extends this with `writeBatch`, `trace`, `affected`, `audit`, and `sourceMap` state. This is a **promote + extend**, not a greenfield build.
- **PluginRegistry exists** at `suites/infrastructure/plugin-registry.concept` with `discover`, `getDefinitions`, `createInstance` etc. GenerationPlan can query this directly.
- **Cache concept exists** at `suites/infrastructure/cache.concept` (tag-based `set`/`get`/`invalidate`). BuildCache is a different concept — specialized for generation step hashes, not general caching.
...
Create `suites/generation/emitter.concept` — the full spec from architecture doc Section 1.5. This extends the existing Clef Bind Emitter with:
...
Create `suites/generation/implementations/typescript/emitter.impl.ts`.
...
Create `suites/generation/syncs/format-after-write.sync` and `suites/generation/syncs/format-batch-after-write.sync` — from doc Section 2.6.

### Step 1.6 — Write stub `suite.yaml`
Create `suites/generation/suite.yaml` with just the Emitter concept and format syncs. We'll add concepts incrementally as each phase completes.
...
Create `suites/generation/build-cache.concept` — full spec from doc Section 1.3. Actions: `check`, `record`, `invalidate`, `invalidateBySource`, `invalidateByKind`, `invalidateAll`, `status`, `staleSteps`.
...
Add BuildCache concept to `suites/generation/suite.yaml`.
...
Create `suites/generation/resource.concept` — full spec from doc Section 1.1. Actions: `upsert`, `get`, `list`, `remove`, `diff`.
...
Create in `suites/generation/syncs/`:
...
Create `suites/generation/kind-system.concept` — full spec from doc Section 1.2. Actions: `define`, `connect`, `route`, `validate`, `dependents`, `producers`, `consumers`, `graph`.
...
Create `suites/generation/generation-plan.concept` — full spec from doc Section 1.4. Actions: `plan`, `begin`, `recordStep`, `complete`, `status`, `summary`, `diff`, `history`.
...
Create `suites/generation/syncs/clean-orphans-after-run.sync` (GenerationPlan/complete → Emitter/clean).
...
Add to `suites/generation/tests/conformance/emitter.test.ts` (already created in Phase 1):
...
### Sync specs (14 files in `suites/generation/syncs/`)
**Required (6):**
- `file-changed.sync`
- `file-modified.sync`
- `file-removed.sync`
...
### Suite manifest (1 file)
- `suites/generation/suite.yaml`

### TypeScript implementations (5 files in `suites/generation/implementations/typescript/`)
- `resource.impl.ts`
- `kind-system.impl.ts`
- `build-cache.impl.ts`
...
### Tests (9 files)
**Conformance (5 in `suites/generation/tests/conformance/`):**
- `resource.test.ts`
- `kind-system.test.ts`
- `build-cache.test.ts`
...
**Integration (4 in `suites/generation/tests/integration/`):**
...
- **Per-generator syncs** (cache-check wrappers, output routing, cache-record, observer) — these live in each family's suite (framework, interface, deploy), not in the generation suite. The architecture doc says these are mechanical and auto-generatable.
