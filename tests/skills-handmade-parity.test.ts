...
describe('Claude Skills Handmade-vs-Generated Parity', () => {
  ...
  beforeAll(() => {
    // Both dirs point to .claude/skills/ — split by content marker
    const allDirs = listSkillDirs(HANDMADE_SKILLS_DIR);
    const allSkills = allDirs
      .map(d => analyzeSkill(HANDMADE_SKILLS_DIR, d))
      .filter((s): s is SkillStructure => s !== null);

    // Handmade skills: no auto-generated marker in body
    handmadeSkills = allSkills.filter(s => !s.body.includes('Auto-generated by Clef'));
    // Generated skills: have the auto-generated marker
    generatedSkills = allSkills.filter(s => s.body.includes('Auto-generated by Clef'));
  });
  ...
  describe('Statistical parity metrics', () => {
    ...
    it('handmade skills count is within expected range', () => {
      // 9 original handmade + newer hand-written skills (create-widget, etc.)
      expect(handmadeSkills.length).toBeGreaterThanOrEqual(9);
      expect(handmadeSkills.length).toBeLessThanOrEqual(30);
    });

    it('generated skills with workflows have steps', () => {
      // Not all generated skills have workflows — only those with
      // workflow annotations in the manifest. Check that those that do
      // have at least one step.
      const withSteps = generatedSkills.filter(s => s.stepCount > 0);
