...
import { verificationRunHandler } from '../handlers/ts/kits/formal-verification/verification-run.handler.js';
...
describe('VerificationRun Handler', () => {
  ...
  describe('start', () => {
    it('creates a run in "running" status with correct property count', async () => {
      const result = await verificationRunHandler.start({
  ...
  describe('complete', () => {
    it('transitions a running run to "completed" with result counts', async () => {
      const started = await verificationRunHandler.start({
        name: 'Password verification',
        property_ids: JSON.stringify(['prop-1', 'prop-2', 'prop-3']),
      }, storage);

      const result = await verificationRunHandler.complete({
        id: started.id,
    ...
    it('rejects completing a non-running run', async () => {
      ...
      // Complete it first
      await verificationRunHandler.complete({
        id: started.id,
  ...
  describe('timeout', () => {
    it('transitions to "timeout" with partial results and remaining count', async () => {
      ...
      const result = await verificationRunHandler.timeout({
        id: started.id,
    ...
    it('rejects timeout on a non-running run', async () => {
      ...
      await verificationRunHandler.cancel({ id: started.id }, storage);
  ...
  describe('cancel', () => {
    it('transitions a running run to "cancelled"', async () => {
      ...
      const result = await verificationRunHandler.cancel({
        id: started.id,
      }, storage);
  ...
  describe('get_status', () => {
    it('reports running status with zero progress initially', async () => {
      ...
      const result = await verificationRunHandler.get_status({
        id: started.id,
      }, storage);
  ...
  describe('compare', () => {
    it('identifies regressions and improvements between two runs', async () => {
      ...
      const runA = await verificationRunHandler.start({
        name: 'Baseline run',
        property_ids: JSON.stringify(['prop-1', 'prop-2', 'prop-3']),
      }, storage);
      await verificationRunHandler.complete({
      ...
      const runB = await verificationRunHandler.start({
        name: 'Updated run',
        property_ids: JSON.stringify(['prop-1', 'prop-2', 'prop-3']),
      }, storage);
      await verificationRunHandler.complete({
      ...
      const result = await verificationRunHandler.compare({
        run_id_a: runA.id,
        run_id_b: runB.id,
      }, storage);
  ...
  describe('full verification run lifecycle', () => {
    it('starts, checks status, completes, and compares runs end-to-end', async () => {
      ...
      const run1 = await verificationRunHandler.start({
      ...
      const status1 = await verificationRunHandler.get_status({ id: run1.id }, storage);
      ...
      const completed1 = await verificationRunHandler.complete({
        id: run1.id,
        results: JSON.stringify({
      ...
      const run2 = await verificationRunHandler.start({
      ...
      const comparison = await verificationRunHandler.compare({
        run_id_a: run1.id,
        run_id_b: run2.id,
      }, storage);
