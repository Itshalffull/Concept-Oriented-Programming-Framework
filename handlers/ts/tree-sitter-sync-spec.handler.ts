...
const _handler: FunctionalConceptHandler = {
  ...
  register(input: Record<string, unknown>) {
    ...
    if (instanceId) { let p = createProgram(); p = get(p, RELATION, instanceId, 'record'); return completeFrom(p, 'ok', (b) => ({ name: 'TreeSitterSyncSpec', language: 'sync-spec', extensions: JSON.stringify(['.sync']), grammarVersion: '1.0.0', registered: b.record !== null })); }
    const p = createProgram(); return complete(p, 'ok', { name: 'TreeSitterSyncSpec', language: 'sync-spec', extensions: JSON.stringify(['.sync']), grammarVersion: '1.0.0', registered: false }) as StorageProgram<Result>;
  },
};
