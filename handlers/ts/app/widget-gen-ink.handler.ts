// WidgetGenInk — Ink terminal UI components with Box/Text
// Uses the RenderProgram pipeline: parses widget AST -> builds instructions -> interprets to ink code.
...
export const widgetGenInkHandler: ConceptHandler = {
  ...
  async generate(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    // Interpret instructions into ink code
    const { output, trace } = interpretInk(built.instructions, componentName);
