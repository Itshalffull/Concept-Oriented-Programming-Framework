// WidgetGenReact — TypeScript/JSX functional components with typed props interfaces
// Uses the RenderProgram pipeline: parses widget AST -> builds instructions -> interprets to react code.
...
export const widgetGenReactHandler: ConceptHandler = {
  ...
  async generate(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    // Interpret instructions into react code
    const { output, trace } = interpretReact(built.instructions, componentName);
