...
import {
  createProgram, find, put, complete, completeFrom, branch, mapBindings,
  type StorageProgram,
...
const _widgetGenFrameworkHandler: FunctionalConceptHandler = {
  initialize(_input: Record<string, unknown>) {
    ...
    return branch(p,
      (bindings) => bindings.existingId != null,
      (bp) => completeFrom(bp, 'ok', (bindings) => ({ instance: bindings.existingId as string })),
      (bp) => {
        const id = nextId();
        let bp2 = put(bp, 'widget-gen-framework', id, { id, providerRef, target: 'framework' });
        bp2 = put(bp2, 'plugin-registry', `widget-gen-provider:${id}`, { id: `widget-gen-provider:${id}`, pluginKind: 'widget-gen-provider', target: 'framework', providerRef, instanceId: id });
