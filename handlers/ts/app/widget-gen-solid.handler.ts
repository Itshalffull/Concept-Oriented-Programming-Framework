// WidgetGenSolid — SolidJS components with signals
// Uses the RenderProgram pipeline: parses widget AST -> builds instructions -> interprets to solid code.
...
export const widgetGenSolidHandler: ConceptHandler = {
  ...
  async generate(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    // Interpret instructions into solid code
    const { output, trace } = interpretSolid(built.instructions, componentName);
