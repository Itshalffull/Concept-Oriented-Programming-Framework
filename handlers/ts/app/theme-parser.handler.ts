...
function flattenTokens(value: unknown, prefix = '', into: Record<string, string> = {}): Record<string, string> {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (isObject(child)) {
      flattenTokens(child, path, into);
    } else if (Array.isArray(child)) {
      into[path] = JSON.stringify(child);
...
function buildDerivedTokens(ast: Json): Record<string, string> {
  ...
  const directTokens = ast.tokens;
  const colors = ast.colors;

  if (isObject(directTokens)) {
    flattenTokens(directTokens, '', tokens);
  }

  if (isObject(colors)) {
    for (const [key, value] of Object.entries(colors)) {
      tokens[`color.${key}`] = String(value);
    }
  }
  ...
  if (colorScheme) {
    ...
    const activeMode = typeof colorScheme.activeMode === 'string'
      ? colorScheme.activeMode
      : (modes && Object.keys(modes)[0]) || 'light';
    ...
    if (activePalette) {
      ...
      tokens['theme.mode'] = activeMode;
    }
  }

  const density = isObject(ast.density) ? ast.density : undefined;
  if (density) {
    if (typeof density.mode === 'string') {
      tokens['density.mode'] = density.mode;
    }
    if (density.multiplier !== undefined) {
      tokens['density.multiplier'] = String(density.multiplier);
    }
  }

  const typeScale = isObject(ast.typeScale) ? ast.typeScale : undefined;
  if (typeScale && isObject(typeScale.steps)) {
    for (const [key, value] of Object.entries(typeScale.steps)) {
      tokens[`typography.${key}`] = String(value);
    }
  }

  const material = isObject(ast.material) ? ast.material : undefined;
  if (material) {
    if (isObject(material.shadows)) {
      for (const [level, value] of Object.entries(material.shadows)) {
        tokens[`elevation.shadow.${level}`] = String(value);
      }
    }
    if (material.backdrop !== undefined) {
      tokens['surface.backdrop'] = String(material.backdrop);
    }
  }

  const springPhysics = isObject(ast.springPhysics) ? ast.springPhysics : undefined;
  if (springPhysics && isObject(springPhysics.presets)) {
    for (const [preset, value] of Object.entries(springPhysics.presets)) {
      tokens[`motion.spring.${preset}`] = JSON.stringify(value);
    }
  }

  const structuralMotif = isObject(ast.structuralMotif) ? ast.structuralMotif : undefined;
  if (structuralMotif && isObject(structuralMotif.intents)) {
    for (const [intent, value] of Object.entries(structuralMotif.intents)) {
      tokens[`motif.${intent}`] = String(value);
    }
  }

  const imageFilter = isObject(ast.imageFilter) ? ast.imageFilter : undefined;
  if (imageFilter && isObject(imageFilter.presets)) {
    for (const [name, value] of Object.entries(imageFilter.presets)) {
      tokens[`image-filter.${name}`] = JSON.stringify(value);
    }
  }
...
function contrastRatio(a: string, b: string): number | null {
  ...
  const lumA = 0.2126 * relativeLuminance(colorA.r)
    + 0.7152 * relativeLuminance(colorA.g)
    + 0.0722 * relativeLuminance(colorA.b);
  const lumB = 0.2126 * relativeLuminance(colorB.r)
    + 0.7152 * relativeLuminance(colorB.g)
    + 0.0722 * relativeLuminance(colorB.b);
...
function extractContrastPairs(tokens: Record<string, string>): Array<[string, string, string, string]> {
  ...
  const entries: Array<[string, string, string, string]> = [
    ['color.foreground', 'color.background', 'foreground', 'background'],
    ['color.text', 'color.surface', 'text', 'surface'],
    ['color.primary', 'color.onPrimary', 'primary', 'onPrimary'],
    ['color.secondary', 'color.onSecondary', 'secondary', 'onSecondary'],
    ['color.error', 'color.onError', 'error', 'onError'],
  ];

  for (const [fgKey, bgKey, fgName, bgName] of entries) {
    if (tokens[fgKey] && tokens[bgKey]) {
      pairs.push([tokens[fgKey]!, tokens[bgKey]!, fgName, bgName]);
    }
  }
...
export const themeParserHandler: ConceptHandler = {
  async parse(input, storage) {
    ...
    try {
      ...
      return { variant: 'error', errors: JSON.stringify([errorMessage]) };
    }
    ...
    if (!hasLegacyTokens && !hasExpressiveBlocks) {
      warnings.push('Theme source has no recognized legacy or expressive theme blocks.');
    }

    for (const [key, value] of Object.entries(parsed)) {
      if (value === null || value === undefined) {
        errors.push(`Theme block "${key}" has null or undefined value`);
      }
    }

    if (errors.length > 0) {
      return { variant: 'error', errors: JSON.stringify(errors) };
    }
    ...
    const context = {
      ...
      motif: typeof structuralMotif.default === 'string'
        ? structuralMotif.default
        : (isObject(structuralMotif.intents) && typeof structuralMotif.intents.navigation === 'string'
          ? structuralMotif.intents.navigation
          : 'default'),
    };
    ...
    const ast = {
      ...
      context,
      sourceType: hasExpressiveBlocks ? 'expressive-theme' : 'legacy-theme',
      raw: parsed,
    };
    ...
    return { variant: 'ok', ast: JSON.stringify(ast) };
  },
  ...
  async checkContrast(input, storage) {
    ...
    for (const [foreground, background, fgName, bgName] of extractContrastPairs(tokens)) {
      ...
      if (ratio !== null && ratio < 4.5) {
        failures.push(`Contrast failure: ${fgName}/${bgName} ratio ${ratio.toFixed(2)} is below 4.5`);
      }
    }

    return failures.length > 0
      ? { variant: 'violations', failures: JSON.stringify(failures) }
      : { variant: 'ok' };
  },
};
