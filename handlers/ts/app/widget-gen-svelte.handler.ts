// WidgetGenSvelte — Svelte components with <script> and template
// Uses the RenderProgram pipeline: parses widget AST -> builds instructions -> interprets to svelte code.
...
export const widgetGenSvelteHandler: ConceptHandler = {
  ...
  async generate(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    // Interpret instructions into svelte code
    const { output, trace } = interpretSvelte(built.instructions, componentName);
