...
const _handler: FunctionalConceptHandler = {
  register() {
    return complete(createProgram(), 'ok', { name: 'StakeWeight' }) as StorageProgram<Result>;
  },
  ...
  stake(input: Record<string, unknown>) {
    ...
    const numAmount = typeof amount === 'string' ? parseFloat(amount as string) : (amount as number);
...
export const stakeWeightHandler = autoInterpret(_handler);
