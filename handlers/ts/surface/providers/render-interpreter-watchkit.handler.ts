...
const _renderInterpreterWatchkitHandler: FunctionalConceptHandler = {
  initialize(input: Record<string, unknown>) {
    ...
    p = branch(p, 'existing',
      ...
      (b) => {
        const id = nextId();
        let b2 = put(b, 'render-interpreter-watchkit', id, {
          id, providerRef: PROVIDER_REF, target: 'watchkit',
        });
        b2 = put(b2, 'plugin-registry', PROVIDER_REF, {
          pluginKind: 'render-interpreter-provider',
          target: 'watchkit', providerRef: PROVIDER_REF, instanceId: id,
        });
...
export const renderInterpreterWatchkitHandler = autoInterpret(_renderInterpreterWatchkitHandler);
