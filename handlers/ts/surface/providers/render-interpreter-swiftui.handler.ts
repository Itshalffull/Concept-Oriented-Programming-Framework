...
export const renderInterpreterSwiftuiHandler: ConceptHandler = {
  async initialize(_input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    const id = nextId();
    await storage.put('render-interpreter-swiftui', id, { id, providerRef: PROVIDER_REF, target: 'swiftui' });
  ...
  async interpret(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    const { output, trace } = interpretSwiftUI(instructions, componentName);
    ...
    if (executionId) {
      await storage.put('executions', executionId, {
        target: 'swiftui', componentName, output, trace, status: 'completed',
      });
    }
