...
export const renderInterpreterComposeHandler: ConceptHandler = {
  async initialize(_input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    const id = nextId();
    await storage.put('render-interpreter-compose', id, { id, providerRef: PROVIDER_REF, target: 'compose' });
  ...
  async interpret(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    const { output, trace } = interpretCompose(instructions, componentName);
    ...
    if (executionId) {
      await storage.put('executions', executionId, {
        target: 'compose', componentName, output, trace, status: 'completed',
      });
    }
