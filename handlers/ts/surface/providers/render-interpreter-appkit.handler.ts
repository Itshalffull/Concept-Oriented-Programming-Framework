...
import { interpretAppKit } from '../interpreter-targets/appkit.ts';
import type { RenderInstruction } from '../render-program-builder.ts';
import { autoInterpret } from '../../../../runtime/functional-compat.ts';

const PROVIDER_REF = 'render-interpreter-provider:appkit';

let idCounter = 0;
function nextId(): string { return `ri-appkit-${++idCounter}`; }

const _renderInterpreterAppkitHandler: FunctionalConceptHandler = {
  initialize(input: Record<string, unknown>) {
    ...
    p = branch(p, 'existing',
      ...
      (b) => {
        const id = nextId();
        let b2 = put(b, 'render-interpreter-appkit', id, {
          id, providerRef: PROVIDER_REF, target: 'appkit',
        });
        b2 = put(b2, 'plugin-registry', PROVIDER_REF, {
          pluginKind: 'render-interpreter-provider',
          target: 'appkit', providerRef: PROVIDER_REF, instanceId: id,
        });
  ...
  interpret(input: Record<string, unknown>) {
    ...
    const { output, trace } = interpretAppKit(instructions, componentName);
    ...
    if (!dryRun) {
      ...
      if (executionId) {
        p = put(p, 'executions', executionId, {
          target: 'appkit', componentName, output, trace, status: 'completed',
        });
      }
    }
...
export const renderInterpreterAppkitHandler = autoInterpret(_renderInterpreterAppkitHandler);
