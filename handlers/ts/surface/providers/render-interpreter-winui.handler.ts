...
const _renderInterpreterWinuiHandler: FunctionalConceptHandler = {
  initialize(input: Record<string, unknown>) {
    ...
    p = branch(p, 'existing',
      ...
      (b) => {
        const id = nextId();
        let b2 = put(b, 'render-interpreter-winui', id, {
          id, providerRef: PROVIDER_REF, target: 'winui',
        });
        b2 = put(b2, 'plugin-registry', PROVIDER_REF, {
          pluginKind: 'render-interpreter-provider',
          target: 'winui', providerRef: PROVIDER_REF, instanceId: id,
        });
...
export const renderInterpreterWinuiHandler = autoInterpret(_renderInterpreterWinuiHandler);
