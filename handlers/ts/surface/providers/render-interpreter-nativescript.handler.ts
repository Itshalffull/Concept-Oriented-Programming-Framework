...
const _renderInterpreterNativescriptHandler: FunctionalConceptHandler = {
  initialize(input: Record<string, unknown>) {
    ...
    p = branch(p, 'existing',
      ...
      (b) => {
        const id = nextId();
        let b2 = put(b, 'render-interpreter-nativescript', id, {
          id, providerRef: PROVIDER_REF, target: 'nativescript',
        });
        b2 = put(b2, 'plugin-registry', PROVIDER_REF, {
          pluginKind: 'render-interpreter-provider',
          target: 'nativescript', providerRef: PROVIDER_REF, instanceId: id,
        });
...
export const renderInterpreterNativescriptHandler = autoInterpret(_renderInterpreterNativescriptHandler);
