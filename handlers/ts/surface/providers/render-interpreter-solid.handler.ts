...
import { interpretSolid } from '../interpreter-targets/solid.ts';
import type { RenderInstruction } from '../render-program-builder.ts';
import { autoInterpret } from '../../../../runtime/functional-compat.ts';

const PROVIDER_REF = 'render-interpreter-provider:solid';

let idCounter = 0;
function nextId(): string { return `ri-solid-${++idCounter}`; }

const _renderInterpreterSolidHandler: FunctionalConceptHandler = {
  initialize(input: Record<string, unknown>) {
    ...
    p = branch(p, 'existing',
      ...
      (b) => {
        const id = nextId();
        let b2 = put(b, 'render-interpreter-solid', id, {
          id, providerRef: PROVIDER_REF, target: 'solid',
        });
        b2 = put(b2, 'plugin-registry', PROVIDER_REF, {
          pluginKind: 'render-interpreter-provider',
          target: 'solid', providerRef: PROVIDER_REF, instanceId: id,
        });
  ...
  interpret(input: Record<string, unknown>) {
    ...
    const { output, trace } = interpretSolid(instructions, componentName);
    ...
    if (!dryRun) {
      ...
      if (executionId) {
        p = put(p, 'executions', executionId, {
          target: 'solid', componentName, output, trace, status: 'completed',
        });
      }
    }
...
export const renderInterpreterSolidHandler = autoInterpret(_renderInterpreterSolidHandler);
