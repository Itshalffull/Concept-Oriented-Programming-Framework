...
export const renderInterpreterVueHandler: ConceptHandler = {
  async initialize(_input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    const id = nextId();
    await storage.put('render-interpreter-vue', id, { id, providerRef: PROVIDER_REF, target: 'vue' });
  ...
  async interpret(input: Record<string, unknown>, storage: ConceptStorage) {
    ...
    const { output, trace } = interpretVue(instructions, componentName);
    ...
    if (executionId) {
      await storage.put('executions', executionId, {
        target: 'vue', componentName, output, trace, status: 'completed',
      });
    }
