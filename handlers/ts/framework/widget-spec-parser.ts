...
class WidgetSpecParser {
  ...
  // Same grammar as the concept parser for structured invariants, with
  // widget-specific divergences (dotted part-method names, positional
  // args, mandatory `->variant` arrow) selected via WIDGET_OPTIONS.
  // See handlers/ts/framework/invariant-body-parser.ts (MAG-910).
  // ================================================================

  private widgetInvariantContext(): AssertionContext {
    return {
      resolveIdentifier: (name: string) => {
        // Widget invariants reference anatomy parts and FSM event names.
        // The parser only consults this for diagnostics; semantic
        // validation is handled by WidgetAssertionContext plugins later.
        return { kind: 'part', part: name };
      },
      declaredSymbols: () => [],
    };
  }
  ...
  private makeInvariantBodyParser(): InvariantBodyParser {
    const stream: TokenStream = {
      ...
      match: (type: string, value?: string) => {
        // Widget parser's .match() returns boolean without consuming;
        // the shared helper expects concept-parser semantics (consume on
        // match, return Token|null). Reimplement locally against the
        // underlying token array.
        const tok = this.peek();
        if (tok.type === (type as TokenType) && (value === undefined || tok.value === value)) {
          return this.advance() as unknown as BasicToken;
        }
        return null;
      },
      expectIdent: () => {
        ...
        throw new Error(
          `Widget parse error at line ${tok.line}: expected identifier, got ${tok.type}(${tok.value})`,
        );
      },
      skipSeps: () => { /* widget grammar has no separator tokens in invariants */ },
      position: () => this.pos,
      seek: (pos: number) => { this.pos = pos; },
    };
    return new InvariantBodyParser(stream, this.widgetInvariantContext(), WIDGET_OPTIONS);
  }
  ...
  private parseInvariant(): InvariantDecl[] {
    this.expect('LBRACE');
    return this.makeInvariantBodyParser().parseInvariantBlock(/*legacyProseStrings*/ true);
  }
}
