...
export function renderTypeScriptTests(plan: TestPlan): string {
  ...
  // Header
  const styleLabel = style === 'external' ? 'External' : style === 'imperative' ? 'Imperative' : 'Functional';
  ...
  if (style === 'external') {
    lines.push('// Validates perform() transport effects, field transforms,');
    lines.push('// mock provider round-trips, and ProcessSpec integration test generation.');
    lines.push(`// External source: ${plan.externalConfig?.source ?? 'unknown'}`);
    lines.push(`// Test tiers: ${plan.testTiers?.join(', ') ?? 'structural'}`);
  ...
  // External handler test tiers (only when handlerStyle is 'external')
  if (plan.handlerStyle === 'external' && plan.externalConfig) {
    lines.push(...renderExternalStructuralTests(handlerVar, plan));
    lines.push(...renderExternalBehavioralTests(handlerVar, plan));
    lines.push(...renderExternalIntegrationTests(plan));
  }
...
// ── External Handler Test Tiers ─────────────────────────────

/**
 * Tier 1: Structural tests — verify correct perform('http', ...) instructions
 * are emitted by the external handler without executing any HTTP calls.
 */
function renderExternalStructuralTests(handlerVar: string, plan: TestPlan): string[] {
  if (!plan.externalConfig || !plan.testTiers?.includes('structural')) return [];
  const lines: string[] = [];
  const config = plan.externalConfig;

  lines.push(`  describe('external handler — Tier 1: structural (dry-run)', () => {`);

  for (const [actionName, mapping] of Object.entries(config.actionMappings)) {
    lines.push(`    it('${actionName} emits ${mapping.method} ${mapping.path}', () => {`);
    lines.push(`      const program = ${handlerVar}.${actionName}({`);

    // Generate sample input from the action's ok fixture
    const action = plan.actions.find(a => a.name === actionName);
    const okFixture = action?.fixtures.find(f => f.expectedVariant === 'ok');
    if (okFixture) {
      for (const [k, v] of Object.entries(okFixture.input)) {
        lines.push(`        ${k}: ${JSON.stringify(v)},`);
      }
    }

    lines.push(`      });`);
    lines.push(`      // Verify the program contains a perform instruction`);
    lines.push(`      const performs = program.instructions.filter(`);
    lines.push(`        (i: any) => i.tag === 'perform' && i.protocol === 'http'`);
    lines.push(`      );`);
    lines.push(`      expect(performs.length).toBeGreaterThan(0);`);
    lines.push(`      expect(performs[0].operation).toBe(${JSON.stringify(mapping.method)});`);

    if (!mapping.path.includes('{{')) {
      lines.push(`      expect(performs[0].payload.path).toBe(${JSON.stringify(mapping.path)});`);
    } else {
      lines.push(`      expect(performs[0].payload.path).toContain('${mapping.path.split('{{')[0]}');`);
    }
...
/**
 * Tier 2: Behavioral tests — full round-trip with mock HTTP responses.
 * Verifies field transforms produce correct concept output from API responses.
 */
function renderExternalBehavioralTests(handlerVar: string, plan: TestPlan): string[] {
  if (!plan.externalConfig || !plan.testTiers?.includes('behavioral')) return [];
  const lines: string[] = [];
  const config = plan.externalConfig;

  lines.push(`  describe('external handler — Tier 2: behavioral (mock provider)', () => {`);

  for (const [actionName, mapping] of Object.entries(config.actionMappings)) {
    const mockResponse = config.mockResponses?.[actionName];
    if (!mockResponse) continue;

    lines.push(`    it('${actionName} transforms ${config.source} response to concept fields', async () => {`);
    lines.push(`      const mockResponse = ${JSON.stringify(mockResponse)};`);
    lines.push(`      // Verify response field mapping`);

    if (mapping.response) {
      for (const [conceptField, apiPath] of Object.entries(mapping.response)) {
        if (conceptField.startsWith('_')) continue;
        lines.push(`      // ${conceptField} ← ${apiPath}`);
        const pathParts = apiPath.split('.');
        let accessor = 'mockResponse';
        for (const part of pathParts) {
          accessor += part.match(/^\d+$/) ? `[${part}]` : `.${part}`;
        }
...
/**
 * Tier 3: Integration test stub — generates a ProcessSpec reference.
 * Actual ProcessSpec generation is done by IntegrationTestGen concept.
 */
function renderExternalIntegrationTests(plan: TestPlan): string[] {
  if (!plan.externalConfig || !plan.testTiers?.includes('integration')) return [];
  const lines: string[] = [];

  lines.push(`  describe('external handler — Tier 3: integration (ProcessSpec)', () => {`);
  lines.push(`    it('generates ProcessSpec from fixtures for ${plan.externalConfig.source}', () => {`);
  lines.push(`      // Integration tests are generated by IntegrationTestGen concept`);
  lines.push(`      // from the concept spec fixtures. Each fixture becomes a process`);
  lines.push(`      // step with type "external-call" that runs against the real API.`);
  lines.push(`      //`);
  lines.push(`      // To run: invoke IntegrationTestGen/generate with:`);
  lines.push(`      //   conceptSpec: "${plan.conceptRef}"`);
  lines.push(`      //   source: "${plan.externalConfig.source}"`);
  lines.push(`      //   ingestManifest: <app.external.yaml content>`);
  lines.push(`      //`);
  lines.push(`      // Then: invoke IntegrationTestGen/run with the generated ProcessSpec.`);
  lines.push(`      //`);
  lines.push(`      // Fixture count: ${plan.actions.reduce((sum, a) => sum + a.fixtures.length, 0)} fixtures across ${plan.actions.length} actions`);
  lines.push(`      expect(true).toBe(true); // placeholder — actual execution via ProcessSpec/start`);
  lines.push(`    });`);
  lines.push(`  });`);
  lines.push('');
  return lines;
