// ============================================================
// Svelte Widget Test Renderer
//
// Renders a WidgetTestPlan into a complete vitest + Svelte Testing
...
//
// Svelte Testing Library differs from React Testing Library in that:
//   - Components are rendered via render(Component, { props: { ... } })
//     rather than JSX — no React/JSX import required
//   - Props are passed as a plain object under the `props` key
//   - Everything else (screen, fireEvent, userEvent, expect matchers)
//     follows the same @testing-library/* conventions
//
...
/**
 * Build a render call for a Svelte component with optional props.
 * Svelte Testing Library uses render(Component, { props: { ... } })
 * rather than JSX, so all "prop" passing goes through this helper.
 */
function renderCall(componentName: string, props: Record<string, string> = {}): string {
  const propEntries = Object.entries(props);
  if (propEntries.length === 0) {
    return `render(${componentName});`;
  }
  const propsStr = propEntries.map(([k, v]) => `${k}: ${v}`).join(', ');
  return `render(${componentName}, { props: { ${propsStr} } });`;
}
...
function renderConnectBindings(
  ...
  for (const a of assertions) {
    ...
    if (stateCondition !== undefined) {
      ...
      body = [
        renderCall(componentName, props),
...
function renderFocusManagement(
  ...
  for (const a of assertions) {
    ...
    if (type === 'trap') {
      body = [
        renderCall(componentName, { open: 'true' }),
...
function renderProps(
  ...
  for (const a of assertions) {
    ...
    if (defaultValue !== undefined && desc.includes('defaults to')) {
      // Default value assertion — render without the prop to see default behaviour
      ...
      // In Svelte Testing Library props are passed as { props: { name: value } }
      body = [
        `const testValue = 'test-${name}-value';`,
        `render(${componentName}, { props: { ${name}: testValue } });`,
...
function renderImports(
  ...
  // Svelte Testing Library exports render and screen from @testing-library/svelte
  const stlImports = ['render', 'screen'];
  if (needsFireEvent) stlImports.push('fireEvent');

  const lines = [
    `import { describe, it, expect } from 'vitest';`,
    `import { ${stlImports.join(', ')} } from '@testing-library/svelte';`,
  ];
  ...
  // Svelte components are imported as default exports (no JSX wrapping needed)
...
/**
 * Render a WidgetTestPlan into a complete vitest + Svelte Testing Library
 * test file string.
 *
 * Components are rendered via `render(Component, { props: { ... } })` rather
 * than JSX — there is no React/JSX dependency. All screen query and event
 * patterns are identical to React Testing Library.
 *
 * @param plan - The test plan produced by WidgetComponentTest/buildPlan
 * @param componentName - The Svelte component name (e.g. 'Dialog')
 * @param componentImportPath - The import path for the component (e.g. '../components/Dialog.svelte')
 * @returns A complete vitest test file as a string
 */
export function renderSvelteWidgetTests(
  plan: WidgetTestPlan,
  componentName: string,
  componentImportPath: string,
): string {
  ...
  // File header comment
  allLines.push(`// Generated Svelte Testing Library tests for ${componentName}`);
