...
export interface FieldsetResult { element: HTMLElement; dispose: () => void; }

export function Fieldset(props: FieldsetProps): FieldsetResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'fieldset');
