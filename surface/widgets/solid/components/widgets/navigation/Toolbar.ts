...
export interface ToolbarResult { element: HTMLElement; dispose: () => void; }

export function Toolbar(props: ToolbarProps): ToolbarResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'toolbar');
  ...
  root.setAttribute('aria-orientation', (props.orientation as string) || 'horizontal');
