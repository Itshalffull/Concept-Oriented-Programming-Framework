...
export interface NavigationMenuResult { element: HTMLElement; dispose: () => void; }

export function NavigationMenu(props: NavigationMenuProps): NavigationMenuResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'navigation-menu');
