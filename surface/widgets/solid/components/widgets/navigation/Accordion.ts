...
export interface AccordionResult { element: HTMLElement; dispose: () => void; }

export function Accordion(props: AccordionProps): AccordionResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'accordion');
