...
export interface BreadcrumbResult { element: HTMLElement; dispose: () => void; }

export function Breadcrumb(props: BreadcrumbProps): BreadcrumbResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'breadcrumb');
