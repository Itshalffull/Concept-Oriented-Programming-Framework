...
export interface CommandPaletteResult { element: HTMLElement; dispose: () => void; }

export function CommandPalette(props: CommandPaletteProps): CommandPaletteResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'command-palette');
