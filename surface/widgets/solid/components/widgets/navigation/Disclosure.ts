...
export interface DisclosureResult { element: HTMLElement; dispose: () => void; }

export function Disclosure(props: DisclosureProps): DisclosureResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'disclosure');
