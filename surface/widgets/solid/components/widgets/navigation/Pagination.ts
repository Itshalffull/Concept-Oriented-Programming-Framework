...
export interface PaginationResult { element: HTMLElement; dispose: () => void; }

export function Pagination(props: PaginationProps): PaginationResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'pagination');
