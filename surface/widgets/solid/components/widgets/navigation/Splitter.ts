...
export interface SplitterResult { element: HTMLElement; dispose: () => void; }

export function Splitter(props: SplitterProps): SplitterResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'splitter');
