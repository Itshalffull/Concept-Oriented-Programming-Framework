...
export interface SidebarResult { element: HTMLElement; dispose: () => void; }

export function Sidebar(props: SidebarProps): SidebarResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'sidebar');
  ...
  root.setAttribute('data-placement', (props.placement as string) || 'left');
