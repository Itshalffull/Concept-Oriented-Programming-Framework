...
export interface FormResult { element: HTMLElement; dispose: () => void; }

export function Form(props: FormProps): FormResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'form');
  ...
  formEl.setAttribute('data-part', 'form');
