...
export interface FloatingToolbarResult { element: HTMLElement; dispose: () => void; }

export function FloatingToolbar(props: FloatingToolbarProps): FloatingToolbarResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'floating-toolbar');
