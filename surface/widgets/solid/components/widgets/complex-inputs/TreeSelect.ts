...
export interface TreeSelectResult { element: HTMLElement; dispose: () => void; }

export function TreeSelect(props: TreeSelectProps): TreeSelectResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'tree-select');
