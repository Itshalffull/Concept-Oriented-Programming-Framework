...
export interface FormulaEditorResult { element: HTMLElement; dispose: () => void; }

export function FormulaEditor(props: FormulaEditorProps): FormulaEditorResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'formula-editor');
