...
export interface PinInputResult { element: HTMLElement; dispose: () => void; }

export function PinInput(props: PinInputProps): PinInputResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'pin-input');
  ...
  root.setAttribute('aria-label', String(props.label || 'PIN input'));
