...
export interface MentionInputResult { element: HTMLElement; dispose: () => void; }

export function MentionInput(props: MentionInputProps): MentionInputResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'mention-input');
