...
export interface SignaturePadResult { element: HTMLElement; dispose: () => void; }

export function SignaturePad(props: SignaturePadProps): SignaturePadResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'signature-pad');
  ...
  canvas.setAttribute('data-part', 'canvas');
