...
export interface RangeSliderResult { element: HTMLElement; dispose: () => void; }

export function RangeSlider(props: RangeSliderProps): RangeSliderResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'range-slider');
  ...
  root.setAttribute('data-orientation', (props.orientation as string) || 'horizontal');
