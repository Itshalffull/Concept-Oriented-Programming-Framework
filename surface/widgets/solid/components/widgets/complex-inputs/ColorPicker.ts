...
export interface ColorPickerResult { element: HTMLElement; dispose: () => void; }

export function ColorPicker(props: ColorPickerProps): ColorPickerResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'color-picker');
