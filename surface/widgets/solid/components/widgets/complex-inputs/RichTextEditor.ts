...
export interface RichTextEditorResult { element: HTMLElement; dispose: () => void; }

export function RichTextEditor(props: RichTextEditorProps): RichTextEditorResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'rich-text-editor');
  ...
  root.setAttribute('aria-label', String(props.label || 'Rich text editor'));
