...
export interface DateRangePickerResult { element: HTMLElement; dispose: () => void; }

export function DateRangePicker(props: DateRangePickerProps): DateRangePickerResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'date-range-picker');
