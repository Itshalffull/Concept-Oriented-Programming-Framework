...
export interface RatingResult { element: HTMLElement; dispose: () => void; }

export function Rating(props: RatingProps): RatingResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'rating');
  ...
  root.setAttribute('aria-label', String(props.label || 'Rating'));
