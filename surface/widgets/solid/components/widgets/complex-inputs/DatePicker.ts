...
export interface DatePickerResult { element: HTMLElement; dispose: () => void; }

export function DatePicker(props: DatePickerProps): DatePickerResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'date-picker');
