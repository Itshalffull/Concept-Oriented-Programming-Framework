...
export interface ContextMenuResult { element: HTMLElement; dispose: () => void; }

export function ContextMenu(props: ContextMenuProps): ContextMenuResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'context-menu');
