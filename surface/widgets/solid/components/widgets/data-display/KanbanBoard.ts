...
export interface KanbanBoardResult { element: HTMLElement; dispose: () => void; }

export function KanbanBoard(props: KanbanBoardProps): KanbanBoardResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'kanban-board');
  ...
  root.setAttribute('aria-label', 'Kanban board');
