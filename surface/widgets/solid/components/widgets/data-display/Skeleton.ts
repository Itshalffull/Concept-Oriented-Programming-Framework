...
export interface SkeletonResult { element: HTMLElement; dispose: () => void; }

export function Skeleton(props: SkeletonProps): SkeletonResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'skeleton');
  ...
  root.setAttribute('data-animation', 'pulse');
  setState('loading');
