...
export interface TimelineResult { element: HTMLElement; dispose: () => void; }

export function Timeline(props: TimelineProps): TimelineResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'timeline');
