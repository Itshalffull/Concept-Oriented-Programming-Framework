...
export interface StatCardResult { element: HTMLElement; dispose: () => void; }

export function StatCard(props: StatCardProps): StatCardResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'stat-card');
