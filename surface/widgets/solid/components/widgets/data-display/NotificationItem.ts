...
export interface NotificationItemResult { element: HTMLElement; dispose: () => void; }

export function NotificationItem(props: NotificationItemProps): NotificationItemResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'notification-item');
