...
export interface CardResult { element: HTMLElement; dispose: () => void; }

export function Card(props: CardProps): CardResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'card');
  ...
  const variant = (props.variant as string) || 'elevated';
  root.setAttribute('data-variant', variant);
  root.setAttribute('data-clickable', String(!!props.clickable));
  root.setAttribute('role', props.clickable ? 'article' : 'region');
  if (props.clickable) root.tabIndex = 0;
