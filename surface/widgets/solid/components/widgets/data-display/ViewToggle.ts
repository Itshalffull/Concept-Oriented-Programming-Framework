...
export interface ViewToggleResult { element: HTMLElement; dispose: () => void; }

export function ViewToggle(props: ViewToggleProps): ViewToggleResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'view-toggle');
