...
export interface ChartResult { element: HTMLElement; dispose: () => void; }

export function Chart(props: ChartProps): ChartResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'chart');
