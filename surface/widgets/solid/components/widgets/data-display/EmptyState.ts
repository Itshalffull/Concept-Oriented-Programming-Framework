...
export interface EmptyStateResult { element: HTMLElement; dispose: () => void; }

export function EmptyState(props: EmptyStateProps): EmptyStateResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'empty-state');
