...
export interface CalendarViewResult { element: HTMLElement; dispose: () => void; }

export function CalendarView(props: CalendarViewProps): CalendarViewResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'calendar-view');
