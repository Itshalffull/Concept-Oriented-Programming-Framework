...
export interface DataTableResult { element: HTMLElement; dispose: () => void; }

export function DataTable(props: DataTableProps): DataTableResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'data-table');
  ...
  table.setAttribute('role', (props.sortable ? 'grid' : 'table'));
  if (props.ariaLabel) table.setAttribute('aria-label', String(props.ariaLabel));
