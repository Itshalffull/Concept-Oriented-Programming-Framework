...
export interface DataListResult { element: HTMLElement; dispose: () => void; }

export function DataList(props: DataListProps): DataListResult {
  const [state, setState] = solidCreateSignal<string>('idle');
  const disposers: Array<() => void> = [];

  const root = document.createElement('div');
  root.setAttribute('data-surface-widget', '');
  root.setAttribute('data-widget-name', 'data-list');
