/* ---------------------------------------------------------------------------
 * ExecutionOverlay -- GTK4/GJS widget
 * Implements the execution-overlay concept widget for GTK4.
...
export function createExecutionOverlay(props: ExecutionOverlayProps): {
  widget: Gtk.Widget;
  update: (nextProps: Partial<ExecutionOverlayProps>) => void;
  dispose: () => void;
} {
  ...
  function send(event: ExecutionOverlayEvent): void {
    state = executionOverlayReducer(state, event);
    render();
  }
  ...
  root.add_css_class('execution-overlay');
  ...
  const headerLabel = new Gtk.Label({ label: 'ExecutionOverlay' });
  headerLabel.add_css_class('execution-overlay-header');
  headerBox.append(headerLabel);

  const stateLabel = new Gtk.Label({ label: '' });
  stateLabel.add_css_class('execution-overlay-state');
  ...
  detailBox.add_css_class('execution-overlay-detail');
  ...
  function render(): void {
    ...
    root.set_tooltip_text(`ExecutionOverlay: ${state}`);
