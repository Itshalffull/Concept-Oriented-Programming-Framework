/* ---------------------------------------------------------------------------
 * VariableInspector -- GTK4/GJS widget
 * Implements the variable-inspector concept widget for GTK4.
...
export function createVariableInspector(props: VariableInspectorProps): {
  widget: Gtk.Widget;
  update: (nextProps: Partial<VariableInspectorProps>) => void;
  dispose: () => void;
} {
  ...
  function send(event: VariableInspectorEvent): void {
  ...
  root.add_css_class('variable-inspector');
  ...
  const headerLabel = new Gtk.Label({ label: 'VariableInspector' });
  headerLabel.add_css_class('variable-inspector-header');
  headerBox.append(headerLabel);

  const stateLabel = new Gtk.Label({ label: '' });
  stateLabel.add_css_class('variable-inspector-state');
  ...
  detailBox.add_css_class('variable-inspector-detail');
  ...
  function render(): void {
    ...
    root.set_tooltip_text(`VariableInspector: ${state}`);
