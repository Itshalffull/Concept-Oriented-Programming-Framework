/* ---------------------------------------------------------------------------
 * RunListTable -- GTK4/GJS widget
 * Implements the run-list-table concept widget for GTK4.
...
export function createRunListTable(props: RunListTableProps): {
  widget: Gtk.Widget;
  update: (nextProps: Partial<RunListTableProps>) => void;
  dispose: () => void;
} {
  ...
  function send(event: RunListTableEvent): void {
  ...
  root.add_css_class('run-list-table');
  ...
  const headerLabel = new Gtk.Label({ label: 'RunListTable' });
  headerLabel.add_css_class('run-list-table-header');
  headerBox.append(headerLabel);

  const stateLabel = new Gtk.Label({ label: '' });
  stateLabel.add_css_class('run-list-table-state');
  ...
  detailBox.add_css_class('run-list-table-detail');
  ...
  function render(): void {
    ...
    root.set_tooltip_text(`RunListTable: ${state}`);
