...
contract HistogramDiffTest is Test {
    ...
    function test_compute_different_same_length() public {
        ...
        assertEq(distance, 0); // same length, distance = 0
        assertTrue(editScript.length > 0); // still produces edit script since content differs
    }
    ...
    function test_compute_no_event_on_identical() public {
        ...
        // No event should be emitted; just verify it returns cleanly
        (bytes memory editScript, uint256 distance) = target.compute(content, content);
        assertEq(distance, 0);
        assertEq(editScript.length, 0);
    }
    ...
    function test_compute_one_empty() public {
        ...
        assertEq(distance, 7); // length of "content"
