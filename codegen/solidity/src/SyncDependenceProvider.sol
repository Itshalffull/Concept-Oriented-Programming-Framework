...
/// @title SyncDependenceProvider
/// @notice Sync dependence analysis provider
/// @dev Implements the SyncDependenceProvider concept from Clef specification.
///      Provides sync-based dependence analysis that can be initialized and registered.

contract SyncDependenceProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("sync", "dependence");
    }

    // --- Actions ---

    /// @notice initialize — create a new sync dependence provider instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("sync", "dependence", block.timestamp, _instanceKeys.length));
