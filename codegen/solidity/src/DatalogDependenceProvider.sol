...
/// @title DatalogDependenceProvider
/// @notice Datalog-based dependence analysis provider
/// @dev Implements the DatalogDependenceProvider concept from Clef specification.
///      Provides Datalog-based dependence analysis that can be initialized and registered.

contract DatalogDependenceProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("datalog", "dependence");
    }

    // --- Actions ---

    /// @notice initialize — create a new Datalog dependence provider instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("datalog", "dependence", block.timestamp, _instanceKeys.length));
