...
/// @title RustBuilder
/// @notice Rust builder provider. Compiles, tests, and packages Rust source code.
contract RustBuilder {
    ...
    struct BuildFeatureConflictResult {
        bool success;
        string[] conflicting;
    }
    ...
    event BuildCompleted(string variant, bytes32 build, bytes[] errors, string[] conflicting);
    ...
    /// @notice build - Compiles Rust source code into an artifact.
    function build(string memory source, string memory toolchainPath, string memory platform, bytes memory config) external returns (BuildOkResult memory) {
        ...
        string memory artifactPath = string(abi.encodePacked(source, "/target/release"));
        ...
        string[] memory emptyConflicting;
        emit BuildCompleted("ok", buildId, emptyErrors, emptyConflicting);
    ...
    /// @notice runTests - Runs tests on a previously built Rust artifact.
