...
/// @title WidgetDependenceProvider
/// @notice Widget dependence analysis provider
/// @dev Implements the WidgetDependenceProvider concept from Clef specification.
///      Provides widget-based dependence analysis that can be initialized and registered.

contract WidgetDependenceProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("widget", "dependence");
    }

    // --- Actions ---

    /// @notice initialize — create a new widget dependence provider instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("widget", "dependence", block.timestamp, _instanceKeys.length));
