...
/// @title SwiftBuilder
/// @notice Swift builder provider. Compiles, tests, and packages Swift source code.
contract SwiftBuilder {
    ...
    /// @notice build - Compiles Swift source code into an artifact.
    function build(string memory source, string memory toolchainPath, string memory platform, bytes memory config) external returns (BuildOkResult memory) {
        ...
        string memory artifactPath = string(abi.encodePacked(source, "/.build/release"));
    ...
    /// @notice runTests - Runs tests on a previously built Swift artifact.
