...
/// @title AppKitAdapter
/// @notice Render adapter for the AppKit framework (macOS native).
///         Normalizes Clef widget props into AppKit-compatible form.
/// @dev Skeleton contract — implement action bodies

contract AppKitAdapter {
    ...
    /// @notice Normalize widget props for the AppKit framework.
    function normalize(bytes32 adapter, string memory props) external returns (NormalizeOkResult memory) {
        ...
        string memory normalized = string(abi.encodePacked("appkit:", props));
