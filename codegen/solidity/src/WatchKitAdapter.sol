...
/// @title WatchKitAdapter
/// @notice Render adapter for the WatchKit framework (Apple Watch).
///         Normalizes Clef widget props into WatchKit-compatible form.
/// @dev Skeleton contract — implement action bodies

contract WatchKitAdapter {
    ...
    /// @notice Normalize widget props for the WatchKit framework.
    function normalize(bytes32 adapter, string memory props) external returns (NormalizeOkResult memory) {
        ...
        string memory normalized = string(abi.encodePacked("watchkit:", props));
