...
/// @title SwiftSdkTarget
/// @notice SDK-target provider that generates Swift SDK packages from concept projections.
/// @dev Produces Swift Package Manager-compatible package structure.

contract SwiftSdkTarget {
    ...
    /// @notice Generate a Swift SDK package from a concept projection
    function generate(string memory projection, string memory config) external returns (GenerateOkResult memory) {
        ...
        files[0] = string(abi.encodePacked("Sources/", projection, "/Client.swift"));
        files[1] = string(abi.encodePacked("Sources/", projection, "/Models.swift"));
        files[2] = "Package.swift";
