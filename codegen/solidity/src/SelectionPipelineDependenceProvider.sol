...
/// @title SelectionPipelineDependenceProvider
/// @notice Selection-pipeline dependence analysis provider
/// @dev Implements the SelectionPipelineDependenceProvider concept from Clef specification.
///      Provides selection-pipeline-based dependence analysis that can be initialized and registered.

contract SelectionPipelineDependenceProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("selection-pipeline", "dependence");
    }

    // --- Actions ---

    /// @notice initialize — create a new selection-pipeline dependence provider instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("selection-pipeline", "dependence", block.timestamp, _instanceKeys.length));
