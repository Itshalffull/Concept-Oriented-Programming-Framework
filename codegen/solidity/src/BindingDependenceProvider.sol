...
/// @title BindingDependenceProvider
/// @notice Binding dependence analysis provider
/// @dev Implements the BindingDependenceProvider concept from Clef specification.
///      Provides binding-based dependence analysis that can be initialized and registered.

contract BindingDependenceProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("binding", "dependence");
    }

    // --- Actions ---

    /// @notice initialize — create a new binding dependence provider instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("binding", "dependence", block.timestamp, _instanceKeys.length));
