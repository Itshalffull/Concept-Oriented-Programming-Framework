...
/// @title NativeScriptAdapter
/// @notice Render adapter for the NativeScript framework.
///         Normalizes Clef widget props into NativeScript-compatible form.
/// @dev Skeleton contract — implement action bodies

contract NativeScriptAdapter {
    ...
    /// @notice Normalize widget props for the NativeScript framework.
    function normalize(bytes32 adapter, string memory props) external returns (NormalizeOkResult memory) {
        ...
        string memory normalized = string(abi.encodePacked("nativescript:", props));
