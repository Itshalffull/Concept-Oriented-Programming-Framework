...
/// @title GTKAdapter
/// @notice Render adapter for the GTK framework (Linux desktop).
///         Normalizes Clef widget props into GTK-compatible form.
...
contract GTKAdapter {
    ...
    /// @notice Normalize widget props for the GTK framework.
    function normalize(bytes32 adapter, string memory props) external returns (NormalizeOkResult memory) {
        ...
        string memory normalized = string(abi.encodePacked("gtk:", props));
