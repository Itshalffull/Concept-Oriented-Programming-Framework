...
/// @title TypeScriptBuilder
/// @notice TypeScript builder provider. Compiles, tests, and packages TypeScript source code.
contract TypeScriptBuilder {
    ...
    struct BuildTypeErrorResult {
        bool success;
        bytes[] errors;
    }

    struct BuildBundleErrorResult {
        bool success;
        string reason;
    }
    ...
    /// @notice build - Compiles TypeScript source code into an artifact.
    function build(string memory source, string memory toolchainPath, string memory platform, bytes memory config) external returns (BuildOkResult memory) {
        ...
        string memory artifactPath = string(abi.encodePacked(source, "/dist"));
    ...
    /// @notice runTests - Runs tests on a previously built TypeScript artifact.
