...
/// @title ReactNativeAdapter
/// @notice Render adapter for the React Native framework.
///         Normalizes Clef widget props into React Native-compatible form.
/// @dev Skeleton contract — implement action bodies

contract ReactNativeAdapter {
    ...
    /// @notice Normalize widget props for the React Native framework.
    function normalize(bytes32 adapter, string memory props) external returns (NormalizeOkResult memory) {
        ...
        string memory normalized = string(abi.encodePacked("react-native:", props));
