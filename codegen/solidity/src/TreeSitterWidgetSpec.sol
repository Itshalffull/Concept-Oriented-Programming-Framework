// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

/// @title TreeSitterWidgetSpec
/// @notice TreeSitter grammar provider for Clef widget specification files
/// @dev Implements the TreeSitterWidgetSpec concept from Clef specification.
///      Provides parsing capabilities for .widget spec files using TreeSitter.

contract TreeSitterWidgetSpec {
    // --- Events ---

    event Registered(string name, string category, string language);
    event Initialized(bytes32 instance);

    // --- Actions ---

    /// @notice Register this grammar provider and return its metadata
    /// @return name The grammar name ("widget-spec")
    /// @return category The provider category ("grammar")
    /// @return language The target language ("widget")
    function register() external pure returns (string memory name, string memory category, string memory language) {
        return ("widget-spec", "grammar", "widget");
    }

    /// @notice Initialize a widget-spec grammar instance
    /// @return instance The unique identifier for this instance
    function initialize() external returns (bytes32 instance) {
        instance = keccak256(abi.encodePacked("widget-spec", block.timestamp, msg.sender));
        emit Initialized(instance);
        return instance;
    }
}
