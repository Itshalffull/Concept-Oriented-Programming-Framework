...
/// @title TypeScriptDependenceProvider
/// @notice TypeScript dependence analysis provider
/// @dev Implements the TypeScriptDependenceProvider concept from Clef specification.
///      Provides TypeScript-specific dependence analysis that can be initialized and registered.

contract TypeScriptDependenceProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("typescript", "dependence");
    }

    // --- Actions ---

    /// @notice initialize — create a new TypeScript dependence provider instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("typescript", "dependence", block.timestamp, _instanceKeys.length));
