...
/// @notice Pattern-match-based analysis engine provider
/// @dev Implements the PatternMatchAnalysisProvider concept from Clef specification.
///      Provides a pattern-matching analysis engine that can be initialized and registered.
...
contract PatternMatchAnalysisProvider {
    ...
    function register() external pure returns (string memory name, string memory category) {
        return ("pattern-match", "analysis-engine");
    }

    // --- Actions ---

    /// @notice initialize — create a new pattern-match analysis engine instance
    function initialize() external returns (InitializeOkResult memory) {
        bytes32 instance = keccak256(abi.encodePacked("pattern-match", "analysis-engine", block.timestamp, _instanceKeys.length));
