...
/// @title NextjsGen
/// @notice Next.js code generator provider for Clef concept specifications
/// @dev Implements the NextjsGen concept from Clef specification.
///      Provider contract: register() is pure and returns static metadata.
///      generate() stores generated Next.js code files keyed by spec ID.
...
contract NextjsGen {
    ...
    /// @return metadata The provider metadata (name, category, language, framework)
    ...
    /// @notice generate - produces Next.js code files from a spec and manifest
    ...
    function generate(bytes32 spec, bytes memory manifest) external returns (GenerateOkResult memory result) {
        require(spec != bytes32(0), "Spec ID cannot be zero");
        require(manifest.length > 0, "Manifest cannot be empty");

        // Generate Next.js files from the manifest
        bytes[] memory files = new bytes[](1);
        files[0] = abi.encodePacked(
            "// Generated Next.js code for spec: ", abi.encodePacked(spec),
